...
for (const removedPrefix of PROVIDER_REMOVED_PATHS) {
    app.use(removedPrefix, (_req, res) => {
        res.status(410).json({
            ...
            message: `Provider endpoint ${removedPrefix} was removed during the Rigorium migration.`,
        });
    });
}
...
app.post('/api/ccr/stats/reset', authenticateToken, (_req, res) => {
    ...
    res.status(501).json({
        ...
        message: 'Per-project router stats reset is not exposed yet; restart the Rigorium server to clear in-memory state.',
    });
});
    ...
    try {
        await startServerAfterStartup({
            ...
            listenFn: async () => {
                ...
                console.log(`${c.info('[INFO]')} Chat execution routed through the Rigorium gateway runtime.`);
                console.log('');
                ...
                process.env.SERVER_PORT = String(boundPort);
                {
                    ...
                    console.log(`  ${c.bright('Rigorium Server - Ready')}`);
                    console.log(c.dim('═'.repeat(63)));
                    console.log('');
                    console.log(`${c.info('[INFO]')} Server URL:  ${c.bright('http://' + DISPLAY_HOST + ':' + boundPort)}`);
                    console.log(`${c.info('[INFO]')} Installed at: ${c.dim(appInstallPath)}`);
                    console.log(`${c.tip('[TIP]')}  Run "rigorium status" for full configuration details`);
                    console.log('');
