...
function showHelp() {
  console.log(`
${c.bright('rigorium - Command Line Tool')}

Usage:
  rigorium [command] [options]

Commands:
  start          Start the Rigorium web UI (default)
...
function showStatus() {
  ...
  console.log('\n' + c.dim('═'.repeat(60)));
  console.log(`\n${c.tip('[TIP]')} Start with ${c.bright('rigorium')} and open http://localhost:${process.env.SERVER_PORT || '3001'}\n`);
}
...
async function main() {
  ...
  switch (command) {
    ...
    default:
      ...
      console.error(`Run ${c.bright('rigorium help')} for usage information.`);
      process.exit(1);
  }
}
