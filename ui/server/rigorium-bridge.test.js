...
describe('gatewayEventToFrames agent status errors', () => {
    it('maps tool result detail availability to a mergeable tool_result frame', () => {
        const frames = gatewayEventToFrames({
            ...
            resultPath: '/tmp/rigorium/tool-result.txt',
        ...
        expect(frames[0]).toMatchObject({
            kind: 'tool_result',
            toolId: 'call-large',
            content: 'Full tool result persisted at /tmp/rigorium/tool-result.txt',
            resultPath: '/tmp/rigorium/tool-result.txt',
        });
    ...
    it('renders gateway unavailable preflight status as an error frame', () => {
        const frames = gatewayEventToFrames({
            ...
            detail: {
                message: 'Rigorium gateway is unavailable.',
        ...
        expect(frames[0]).toMatchObject({
            kind: 'error',
            content: 'Rigorium gateway is unavailable.',
            code: 'gateway_unavailable',
            userHint: 'Start or restart the Rigorium gateway, then retry this message.',
        });
    });
...
describe('isGatewayUnavailableError', () => {
    it('detects cached gateway websocket disconnects', () => {
        ...
        expect(isGatewayUnavailableError(new Error('[rigorium-bridge] gateway connect failed after 60000ms'))).toBe(true);
    });
