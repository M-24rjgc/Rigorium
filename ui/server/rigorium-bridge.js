/**
 * Rigorium bridge — the only chat-execution entry point in `ui/server/`.
 *
 *
 *   1. Connects to the standalone Rigorium gateway server
 *      (`rigorium server`, default ws://127.0.0.1:18789/ws) as a
 *      WebSocket client. We never instantiate an in-process gateway
 *      here — that would create a second, divergent agent runtime that
 *      doesn't share `~/.rigorium/projects/<id>/chats/*.jsonl` writes
 *      and permission state with the CLI/TUI surfaces. One process, one
 *      gateway.
 *   2. Maps each old "sessionId" → Rigorium "sessionKey" (1:1, generated
 ...
 * Two-process launch:
 *
 *   - `rigorium server` (port 18789) owns the gateway, agent loop,
...
import { resolveRigoriumHome, createProjectId, sanitizeSessionIdForPath } from './utils/rigoriumPaths.js';
...
const GENERAL_HOME = resolveRigoriumHome(process.env);

const GATEWAY_URL =
    process.env.RIGORIUM_GATEWAY_URL || 'ws://127.0.0.1:18789/ws';
const GATEWAY_TOKEN_PATH =
    process.env.RIGORIUM_GATEWAY_TOKEN_PATH ||
...
const GATEWAY_CONNECT_TIMEOUT_MS =
    Number.parseInt(process.env.RIGORIUM_BRIDGE_TIMEOUT ?? '', 10) || 60_000;
 ...
 * `default` so Rigorium's `Permission.decide()` fully evaluates rules
 ...
 * Override with `RIGORIUM_WEB_PERMISSION_MODE`.
 */
const WEB_DEFAULT_PERMISSION_MODE =
    process.env.RIGORIUM_WEB_PERMISSION_MODE || 'default';
...
async function connectWithRetry() {
    ...
    while (Date.now() < deadline) {
        ...
        if (token) {
            try {
                ...
                console.log(
                    `[rigorium-bridge] connected → ${GATEWAY_URL}`,
                );
                return gateway;
            } catch (error) {
                lastError = error;
    ...
    throw new Error(
        `[rigorium-bridge] gateway connect failed after ${GATEWAY_CONNECT_TIMEOUT_MS}ms${detail}`,
    );
}
 ...
 */
export async function getRigoriumGateway() {
    return ensureGateway();
}

export function getRigoriumRepoRoot() {
    return REPO_ROOT;
}
 ...
 * @param {string} provider Provider hint (rigorium).
...
/**
 * Run a chat command through the Rigorium gateway.
 *
 * The frontend addresses sessions by the Rigorium `sessionKey` itself
...
export async function runChatViaGateway(
    ...
    const sessionKey = isRigoriumSessionKey(incoming) ? incoming : newSessionKey();
    ...
    console.log(`[rigorium-bridge] submitTurn runMode=${runMode} mode=${resolvedMode} (options.permissionMode=${options?.permissionMode}, options.mode=${options?.mode})`);
    ...
    try {
        ...
        if (staleRunId) {
            ...
            console.log(
                `[rigorium-bridge] ${abortAction} turn ${staleRunId} for ${sessionKey} before submit`,
            );
            try {
                ...
                if (options?.forceStart === true) {
                    ...
                    console.warn('[rigorium-bridge] force-start abort failed:', err?.message || err);
                    writer.send(
                        createNormalizedMessage({
                            provider,
                            sessionId: sessionKey,
                ...
                console.warn('[rigorium-bridge] stale abort failed (continuing):', err?.message || err);
            }
        }
        ...
        for await (const event of stream) {
            ...
            if (event && event.type === 'error') {
                ...
                console.error(
                    '[rigorium-bridge] gateway error event:',
                    JSON.stringify(
                        {
                            sessionKey,
                            projectKey,
        ...
        if (!sawTurnCompleted && !sawGatewayError) {
            ...
            const userHint = 'The model stream ended before Rigorium received a final turn result. Please retry this message; if it repeats, check the gateway/model provider logs.';
            const statusEvent = createBridgeFailureStatusEvent({
                event: 'gateway_stream_ended_without_completion',
                message,
                userHint,
            });
            console.warn(`[rigorium-bridge] ${message}`, { sessionKey, projectKey, runId });
        ...
        const message = gatewayUnavailable ? 'Rigorium gateway is unavailable.' : rawMessage;
        const statusEvent = gatewayUnavailable
            ? createBridgeFailureStatusEvent({
                event: 'gateway_unavailable',
                message,
                userHint: 'Start or restart the Rigorium gateway, then retry this message.',
                scope: 'preflight',
                detail: {
        ...
        console.error(
            '[rigorium-bridge] runChatViaGateway threw:',
...
async function recordGatewayStatusMessage(gateway, { sessionKey, turnId, projectKey, event, text, detail }) {
    ...
    try {
        ...
        console.warn('[rigorium-bridge] failed to record gateway status message:', error?.message || error);
    }
}

export async function abortViaGateway(sessionId, _provider = 'rigorium') {
    const gw = await ensureGateway();
    const sessionKey = isRigoriumSessionKey(sessionId) ? sessionId : null;
    ...
    try {
        ...
        console.warn('[rigorium-bridge] abortTurn failed:', error);
        return false;
    }
}
...
export async function decidePermissionViaGateway(requestId, decision, options = {}) {
    ...
    for (const state of sessionState.values()) {
        try {
            ...
            console.warn('[rigorium-bridge] permissionDecide failed:', error);
        }
    }
    return false;
}
...
export async function grantSessionPermissionViaGateway(sessionId, entry) {
    const gw = await ensureGateway();
    if (!isRigoriumSessionKey(sessionId) || typeof entry !== 'string' || !entry.trim()) {
        return false;
    }
    try {
        ...
        console.warn('[rigorium-bridge] grantSessionPermission failed:', error);
        return false;
    }
}

export function isSessionActiveViaGateway(sessionId) {
    if (!isRigoriumSessionKey(sessionId)) return false;
    return Boolean(sessionState.get(sessionId)?.active);
}

export async function getActiveTurnSnapshotFramesViaGateway(sessionId, provider = 'rigorium') {
    if (!isRigoriumSessionKey(sessionId)) return [];
    const gw = await ensureGateway();
...
/**
 * Read persisted router stats from `~/.rigorium/router/stats.json`.
 * Falls back to the legacy `~/.rigorium/router-stats.json` path.
 *
 ...
 * Scans project chat directories under ~/.rigorium/projects/ and maps
...
function _readFirstPrompt(sessionId, projectKey) {
    const rigoriumHome = GENERAL_HOME;
    ...
    if (projectKey) {
        for (const id of fileVariants) {
            candidates.push(path.join(rigoriumHome, 'projects', createProjectId(projectKey), 'chats', `${id}.jsonl`));
        }
    }
    // Also check the general workspace (sessions may live there)
    for (const id of fileVariants) {
        const generalChatPath = path.join(rigoriumHome, 'projects', createProjectId(rigoriumHome), 'chats', `${id}.jsonl`);
        ...
        for (const record of records) {
            ...
            if (!sessionEntry) {
                sessionEntry = {
                    ...
                    provider: record.provider || 'rigorium',
...
export function registerAlwaysOnNotificationForwarding(clients) {
    ...
    ensureGateway().then((gw) => {
        ...
        console.warn('[rigorium-bridge] failed to register always-on notification forwarding:', err?.message || err);
    });
}
...
export async function elicitationRespondViaGateway(requestId, answer) {
    ...
    for (const state of sessionState.values()) {
        try {
            ...
            console.warn('[rigorium-bridge] respondElicitation failed:', error);
        }
    }
    return false;
}
