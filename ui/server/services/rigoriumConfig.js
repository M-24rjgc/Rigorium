...
import { parseGatewayConfig } from '../../../src/rigorium/config/parseGatewayConfig.js';

// Source of truth: ~/.rigorium/rigorium.yaml. The disk format and the
...
// flows through verbatim — the gateway-side RigoriumConfigStore owns those
// schemas. UI server just round-trips them.

const CONFIG_VERSION = 1;
const RIGORIUM_HOME_DIR = process.env.RIGORIUM_HOME || path.join(os.homedir(), '.rigorium');
...
export function buildDefaultRigoriumConfig() {
  return {
    ...
    webui: {
      runtime: {
        ...
        vitePort: 5173,
        apiTimeoutMs: 120000,
        databasePath: path.join(RIGORIUM_HOME_DIR, 'auth.db'),
...
export function normalizeRigoriumConfig(input) {
  return deepMerge(buildDefaultRigoriumConfig(), isRecord(input) ? input : {});
}
...
export function validateRigoriumConfig(config) {
  const normalized = normalizeRigoriumConfig(config);
  const errors = [];
  const warnings = [];
...
export function buildRuntimeEnv(config) {
  ...
  const env = {
    ...
    RIGORIUM_MEMORY_ENABLED: normalized.memory?.enabled ? '1' : '0',
  };
  ...
  if (main) {
    env.RIGORIUM_API_BASE_URL = main.provider.url || '';
    env.RIGORIUM_API_KEY = main.provider.apiKey || '';
    env.RIGORIUM_MODEL = main.model;
  ...
  if (Number.isFinite(requestedMaxOutput) && requestedMaxOutput > 0) {
    env.RIGORIUM_MAX_OUTPUT_TOKENS = String(requestedMaxOutput);
  } else if (process.env.RIGORIUM_MAX_OUTPUT_TOKENS) {
    env.RIGORIUM_MAX_OUTPUT_TOKENS = process.env.RIGORIUM_MAX_OUTPUT_TOKENS;
  }
  ...
  if (memory) {
    env.RIGORIUM_MEMORY_MODEL = memory.model;
    env.RIGORIUM_MEMORY_PROVIDER = memory.providerId;
    env.RIGORIUM_MEMORY_BASE_URL = memory.provider.url || '';
    env.RIGORIUM_MEMORY_API_KEY = memory.provider.apiKey || '';
    env.RIGORIUM_MEMORY_API_TYPE = normalizeString(normalized.memory?.apiType)
...
export function buildMemoryDefaults(config) {
  const memory = normalizeRigoriumConfig(config).memory ?? {};
  return {
...
export function getRigoriumConfigPath() {
  if (process.env.RIGORIUM_CONFIG_PATH?.trim()) {
    return process.env.RIGORIUM_CONFIG_PATH.trim();
  }
  return DEFAULT_CONFIG_PATH;
}

export function readRigoriumConfigFile() {
  ...
  if (!fs.existsSync(configPath)) {
    return {
      ...
      configPath,
      raw: '',
      config: buildDefaultRigoriumConfig(),
      rawYaml: {},
      parseError: null,
    };
  }
  ...
  const config = normalizeRigoriumConfig(parsed);
...
// (loadRigoriumConfig.ts auto-overrides router.scenarios.default with
...
export async function writeRigoriumConfig(config) {
  ...
  const validation = validateRigoriumConfig(sanitized);
  if (!validation.valid) {
    const error = new Error('Invalid Rigorium config');
...
// writeRigoriumConfig now that internal === disk.
export async function writeRawRigoriumYaml(yamlObj) {
  return writeRigoriumConfig(yamlObj);
}
...
export function parseConfigYaml(raw) {
  return normalizeRigoriumConfig(parseYaml(raw) || {});
}
