...
// or supplied via RIGORIUM_CRON_DAEMON_BIN. Returning `null` falls back
// to the in-tree fallback path that handles missing binaries gracefully.
function resolveRigoriumMainRoot() {
    return null;
}
...
function getCronDaemonStartLockPath() {
  return path.join(getRigoriumConfigHomeDir(), 'cron-daemon', 'start.lock');
}
 ...
 * discarded — making post-mortem debugging on the Rigorium Desktop install
 * basically impossible (`~/.rigorium/desktop.server.log` only captured the
 * UI server's own output, not its detached children).
 *
 * We honour an explicit override via `RIGORIUM_CRON_DAEMON_LOG`; otherwise we
 * default to `~/.rigorium/cron-daemon.log` (parallel to `desktop.server.log`).
 * The directory is created on demand so this works pre-onboarding too.
 */
function resolveCronDaemonLogPath() {
  const override = process.env.RIGORIUM_CRON_DAEMON_LOG?.trim();
  if (override) return override;
  return path.join(process.env.RIGORIUM_HOME || path.join(os.homedir(), '.rigorium'), 'cron-daemon.log');
}
...
export function buildCronDaemonSpawnCommand({
  resolveRigoriumMainRootFn = resolveRigoriumMainRoot,
  cliPath = process.env.RIGORIUM_CLI_PATH
} = {}) {
  const localMainRoot = resolveRigoriumMainRootFn();
  if (localMainRoot) {
  ...
  return {
    command: typeof cliPath === 'string' && cliPath.trim().length > 0 ? cliPath.trim() : 'rigorium',
    args: ['daemon', 'serve']
  };
}
...
export function startCronDaemonDetached({
  ...
  // Detach so multiple ui servers (e.g. dev + Rigorium Desktop side-by-side)
  // can share state through ~/.rigorium/cron-daemon.sock, but pipe stdout/stderr
