...
function useTempConfig(contents, filename = 'rigorium.yaml') {
    const dir = mkdtempSync(join(tmpdir(), 'rigorium-config-test-'));
    tempDirs.push(dir);
...
describe('readRigoriumConfigFile fallback behavior', () => {
    it('disables Office preview by default', () => {
        expect(buildDefaultRigoriumConfig().webui.officePreview).toEqual({
            service: 'none',
            binaryPath: '',
        });
    });
...
describe('validateRigoriumConfig gateway validation', () => {
    it('rejects non-object gateway config', () => {
        const validation = validateRigoriumConfig({ gateway: true });
    ...
    it('rejects unsupported gateway bindAddress', () => {
        const validation = validateRigoriumConfig({
            gateway: {
                bindAddress: '0.0.0.0',
            },
        });
    ...
    it('warns when gateway.tokenPath is configured', () => {
        ...
        expect(validation.warnings).toContain(
            'gateway.tokenPath: gateway.tokenPath is no longer configurable; the gateway token is stored under RigoriumHome.',
        );
    });
