...
} from './rigoriumConfig.js';
import { reloadRigoriumConfig } from './rigoriumConfigReloader.js';

// Watches ~/.rigorium/rigorium.yaml for external edits (vim, Cursor, other IDEs)
...
async function handleChange(configPath) {
  ...
  try {
    reloadResult = await reloadRigoriumConfig(record.config);
  } catch (error) {
    onEventHandler?.({
      source: 'watcher',
      path: record.configPath,
...
export async function startRigoriumConfigWatcher({ onEvent } = {}) {
  ...
  try {
    ...
    console.warn('[rigorium-config-watcher] failed to ensure config dir:', error?.message || error);
    return;
  }
  ...
  try {
    ...
    watcher.on('error', (error) => {
      console.warn('[rigorium-config-watcher] watch error:', error?.message || error);
    });
    console.log(`[rigorium-config-watcher] watching ${configPath}`);
  } catch (error) {
    console.warn('[rigorium-config-watcher] failed to start:', error?.message || error);
  }
}

export function stopRigoriumConfigWatcher() {
  if (watcher) {
    try {
      watcher.close();
    } catch {
