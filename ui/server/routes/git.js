...
router.post('/generate-commit-message', async (req, res) => {
  const { project, files, provider = 'rigorium' } = req.body;
  ...
  // Validate provider
  if (!['claude', 'rigorium', 'cursor'].includes(provider)) {
    return res.status(400).json({ error: 'provider must be "claude", "rigorium" or "cursor"' });
  }
...
Generate the commit message:`;
  ...
  try {
    ...
    const writer = {
      send: (data) => {
        try {
          ...
          if ((parsed.type === 'claude-response' || parsed.type === 'rigorium-response') && parsed.data) {
            const message = parsed.data.message || parsed.data;
            console.log('Rigorium response message:', JSON.stringify(message, null, 2).substring(0, 500));
    ...
    // All providers route through the Rigorium gateway. The `provider`
