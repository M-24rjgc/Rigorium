...
import { useRigoriumConfig, type ConfigReload } from '../../../../hooks/useRigoriumConfig';
import {
...
function fallbackSubsystemStatus(key: SubsystemKey, config: RigoriumConfig | null): { state: StatusState; detailKey: string } {
  if (!config) {
...
function subsystemStatus(key: SubsystemKey, config: RigoriumConfig | null, reload: ConfigReload | null): { state: StatusState; detailKey: string; detail?: string } {
        ...
        return (
          <div key={item.key} className="rounded-lg bg-muted/30 px-3.5 py-2.5">
            <div className="flex items-center gap-2 text-[13px] font-semibold leading-5 text-foreground">
              ...
              <span>{t(`rigoriumConfig.status.subsystems.${item.labelKey}.label`)}</span>
            </div>
            <div className={cn('mt-0.5 text-[11px] leading-4', status.state === 'error' ? 'text-destructive' : 'text-muted-foreground')}>
              {status.detail ?? t(`rigoriumConfig.status.subsystems.${status.detailKey}`)}
            </div>
          </div>
        );
      })}
...
function safeParseYaml(text: string): RigoriumConfig | null {
  try {
    const value = parseYaml(text);
    if (value && typeof value === 'object' && !Array.isArray(value)) return value as RigoriumConfig;
    return null;
  } catch {
    return null;
  }
 ...
 */
function configToYamlString(config: RigoriumConfig): string {
...
function rewriteProviderRefs(config: RigoriumConfig, oldProviderId: string, newProviderId: string): RigoriumConfig {
  let next = config;
...
function replaceFallbackModelRef(config: RigoriumConfig, oldRef: string, newRef: string): RigoriumConfig {
...
function findReusableDeepSeekProvider(config: RigoriumConfig): string | undefined {
...
function getOfficePreviewService(config: RigoriumConfig): OfficePreviewService {
  return normalizeOfficePreviewService(config.webui?.officePreview?.service);
}

function ServiceSection({ config, onChange }: { config: RigoriumConfig; onChange: (next: RigoriumConfig) => void }) {
  const { t } = useTranslation('settings');
  ...
  return (
    <SettingsSection
      title={t('rigoriumConfig.panels.runtime.title')}
      description={t('rigoriumConfig.panels.runtime.description')}
    >
      <SettingsCard>
        <div className="divide-y divide-border">
          <FormRow label={t('rigoriumConfig.panels.runtime.fields.host.label')} description={t('rigoriumConfig.panels.runtime.fields.host.description')}>
            <TextInput value={r.host} placeholder="0.0.0.0" onChange={(v) => set('host', v)} />
          </FormRow>
          <FormRow label={t('rigoriumConfig.panels.runtime.fields.serverPort.label')} description={t('rigoriumConfig.panels.runtime.fields.serverPort.description')}>
            <NumberInput value={r.serverPort} placeholder="3001" onChange={(v) => set('serverPort', v)} />
          </FormRow>
          <FormRow label={t('rigoriumConfig.panels.runtime.fields.workspacesRoot.label')} description={t('rigoriumConfig.panels.runtime.fields.workspacesRoot.description')}>
        ...
        <div className="border-t border-border px-4 py-2.5">
          ...
          >
            ...
            {t('rigoriumConfig.panels.runtime.advancedToggle')}
          </button>
        </div>
        {showAdvanced && (
          <div className="divide-y divide-border border-t border-border">
            <FormRow label={t('rigoriumConfig.panels.runtime.fields.vitePort.label')} description={t('rigoriumConfig.panels.runtime.fields.vitePort.description')}>
              <NumberInput value={r.vitePort} placeholder="5173" onChange={(v) => set('vitePort', v)} />
            </FormRow>
            <FormRow label={t('rigoriumConfig.panels.runtime.fields.apiTimeout.label')} description={t('rigoriumConfig.panels.runtime.fields.apiTimeout.description')}>
              <NumberInput value={r.apiTimeoutMs} placeholder="120000" onChange={(v) => set('apiTimeoutMs', v)} />
            </FormRow>
            <FormRow label={t('rigoriumConfig.panels.runtime.fields.databasePath.label')} description={t('rigoriumConfig.panels.runtime.fields.databasePath.description')}>
              <TextInput value={r.databasePath} placeholder="~/.rigorium/auth.db" monospace onChange={(v) => set('databasePath', v)} />
            </FormRow>
            <FormRow label={t('rigoriumConfig.panels.runtime.fields.proxyUrl.label')} description={t('rigoriumConfig.panels.runtime.fields.proxyUrl.description')}>
              <TextInput value={config.proxy?.url} placeholder="http://127.0.0.1:7890" monospace onChange={(v) => onChange(patch(config, ['proxy', 'url'], v))} />
            </FormRow>
            <FormRow label={t('rigoriumConfig.panels.runtime.fields.proxyNoProxy.label')} description={t('rigoriumConfig.panels.runtime.fields.proxyNoProxy.description')}>
...
function OfficePreviewSection({ config, onChange }: { config: RigoriumConfig; onChange: (next: RigoriumConfig) => void }) {
  const { t } = useTranslation('settings');
  ...
  useEffect(() => {
    ...
    readOfficePreviewStatus({ refresh: statusReloadKey > 0 })
      ...
      .catch((error: Error) => {
        if (cancelled) return;
        setStatusError(error.message || t('rigoriumConfig.panels.officePreview.status.error'));
      })
      .finally(() => {
  ...
  return (
    <SettingsSection
      title={t('rigoriumConfig.panels.officePreview.title')}
      description={t('rigoriumConfig.panels.officePreview.description')}
    >
      <SettingsCard>
        <div className="divide-y divide-border">
          <FormRow
            label={t('rigoriumConfig.panels.officePreview.fields.service.label')}
            description={t('rigoriumConfig.panels.officePreview.fields.service.description')}
          >
            <div className="max-w-xs">
              <Select
                ...
                options={[
                  { value: 'none', label: t('rigoriumConfig.panels.officePreview.options.none') },
                  {
                    value: 'libreoffice',
                    label: libreOfficeUnavailable
                      ? t('rigoriumConfig.panels.officePreview.options.libreOfficeUnavailable')
                      : t('rigoriumConfig.panels.officePreview.options.libreOffice'),
                  },
                ]}
              />
            </div>
          ...
          {service === 'libreoffice' && (
            <FormRow
              label={t('rigoriumConfig.panels.officePreview.fields.binaryPath.label')}
              description={t('rigoriumConfig.panels.officePreview.fields.binaryPath.description')}
            >
                ...
                <div className="flex gap-2">
                  <div className="min-w-0 flex-1">
                    <TextInput
                      ...
                      placeholder={t('rigoriumConfig.panels.officePreview.fields.binaryPath.placeholder')}
                      monospace
                  ...
                  >
                    ...
                    {t('rigoriumConfig.panels.officePreview.scan.button')}
                  </button>
                </div>
                {scanListOpen && (
                  <div className="absolute left-0 right-0 top-full z-20 mt-1 overflow-hidden rounded-lg border border-border bg-popover shadow-lg">
                    <div className="border-b border-border px-3 py-2 text-[12px] font-medium text-foreground">
                      {t('rigoriumConfig.panels.officePreview.scan.title')}
                    </div>
                    <button
                      type="button"
                    ...
                    >
                      <span className="min-w-0 truncate text-foreground">
                        {t('rigoriumConfig.panels.officePreview.options.autoDetect')}
                      </span>
                    ...
                    {statusLoading ? (
                      <div className="flex items-center gap-2 px-3 py-3 text-[12px] text-muted-foreground">
                        ...
                        {t('rigoriumConfig.panels.officePreview.scan.scanning')}
                      </div>
                          ...
                          >
                            <span className="min-w-0">
                              ...
                              <span className={cn(
                                ...
                                {candidate.available
                                  ? t('rigoriumConfig.panels.officePreview.options.candidateAvailableShort')
                                  : t('rigoriumConfig.panels.officePreview.options.candidateUnavailableShort')}
                      ...
                      <div className="border-t border-border px-3 py-3 text-[12px] leading-5 text-muted-foreground">
                        {t('rigoriumConfig.panels.officePreview.status.noDetectedPaths')}
                      </div>
                    )}
                  </div>
                )}
          ...
          <div className="space-y-2 px-4 py-3">
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex min-w-0 items-center gap-2 text-[12px] text-muted-foreground">
                  ...
                  <span className="min-w-0 truncate">
                    {statusLoading
                      ? t('rigoriumConfig.panels.officePreview.status.checking')
                      : statusError
                        ? t('rigoriumConfig.panels.officePreview.status.error')
                        : libreOfficeAvailable
                          ? t('rigoriumConfig.panels.officePreview.status.available')
                          : libreOfficeUnavailable
                            ? t('rigoriumConfig.panels.officePreview.status.unavailable')
                            : libreOfficeUnknown
                              ? t('rigoriumConfig.panels.officePreview.status.unknown')
                              : t('rigoriumConfig.panels.officePreview.status.unavailable')}
                  </span>
                </div>
                <button
                ...
                >
                  ...
                  {t('rigoriumConfig.panels.officePreview.status.refresh')}
                </button>
              </div>
            )}
              ...
              <div className="space-y-1 rounded-md bg-muted/30 px-3 py-2 text-[11px] leading-4 text-muted-foreground">
                {status?.libreOffice?.binaryPath && (
                  <div className="truncate" title={status.libreOffice.binaryPath}>
                    {t('rigoriumConfig.panels.officePreview.status.path', { path: status.libreOffice.binaryPath })}
                  </div>
                )}
                {status?.libreOffice?.version && (
                  <div className="truncate" title={status.libreOffice.version}>
                    {t('rigoriumConfig.panels.officePreview.status.version', { version: status.libreOffice.version })}
                  </div>
                )}
              </div>
            )}
            ...
            {service === 'none' && (
              <div className="rounded-md bg-muted/30 px-3 py-2 text-[11px] leading-4 text-muted-foreground">
                {t('rigoriumConfig.panels.officePreview.disabledNote')}
              </div>
            )}
            ...
            {service === 'libreoffice' && libreOfficeUnavailable && (
              <div className="flex items-start gap-2 rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-[12px] leading-5 text-amber-700 dark:text-amber-300">
                ...
                <div>{t('rigoriumConfig.panels.officePreview.unavailableWarning')}</div>
              </div>
            )}
...
function ProviderCard({
  ...
  const displayName = providerDisplayName(
    ...
    catalogEntry,
    t('rigoriumConfig.panels.models.customProvider'),
  );
  ...
  const commitProviderId = () => {
    ...
    if (onRename(nextId)) {
      ...
      setProviderIdDraft(providerId);
      setProviderIdError(t('rigoriumConfig.panels.models.providerIdDuplicate'));
    }
  };
    ...
    <div className="space-y-3 rounded-lg border border-border bg-background/50 p-4 transition-colors">
      ...
      <div className="flex items-center gap-2">
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1 flex items-center gap-2">
            <span className="text-[11px] text-muted-foreground">{t('rigoriumConfig.panels.models.providerId')}</span>
            <input
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-[160px_1fr]">
        <label className="text-xs text-muted-foreground">
          <span className="mb-1 block">{t('rigoriumConfig.panels.models.protocol')}</span>
          <Select
            value={protocol}
            onChange={(v) => update({ protocol: v as CatalogProviderProtocol })}
            options={[
              { value: 'openai',    label: t('rigoriumConfig.panels.models.protocolOptions.openai') },
              { value: 'openai-responses', label: t('rigoriumConfig.panels.models.protocolOptions.openaiResponses') },
              { value: 'anthropic', label: t('rigoriumConfig.panels.models.protocolOptions.anthropic') },
              { value: 'google',    label: t('rigoriumConfig.panels.models.protocolOptions.google') },
            ]}
          />
        </label>
        <label className="text-xs text-muted-foreground">
          <span className="mb-1 block">{t('rigoriumConfig.panels.models.baseUrl')}</span>
          <TextInput
            value={provider.url}
          ...
          <span className="mt-0.5 block text-[10px] text-muted-foreground/70">
            {t('rigoriumConfig.panels.models.baseUrlHint')}
          </span>
          {!provider.url && catalogEntry && (
            <span className="mt-0.5 block text-[10px] text-muted-foreground/70">
              {t('rigoriumConfig.panels.models.defaultsTo')}{' '}
              <code className="font-mono">{catalogEntry.defaultUrl}</code>
              {' '}{t('rigoriumConfig.panels.models.fromCatalog')}
            </span>
          )}
          {effectiveUrl && provider.url && (
            <span className="mt-0.5 block text-[10px] text-muted-foreground/70">
              {t('rigoriumConfig.panels.models.effective')}{' '}
      ...
      <label className="block text-xs text-muted-foreground">
        <span className="mb-1 block">{t('rigoriumConfig.panels.models.apiKey')}</span>
        <SecretTextInput
          value={provider.apiKey}
          emptyPlaceholder="sk-..."
          maskedPlaceholder={t('rigoriumConfig.panels.models.maskedKeyPlaceholder')}
          onChange={(v) => update({ apiKey: v })}
        />
        {isMaskedKey && (
          <span className="mt-1 inline-flex items-center gap-1 text-[11px] text-muted-foreground">
            <Info className="h-3 w-3" />
            {t('rigoriumConfig.panels.models.keyHidden')}
          </span>
        )}
      </label>
      ...
      <div>
        <div className="mb-1.5 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-xs text-muted-foreground">
          <span>{t('rigoriumConfig.panels.models.enabledModels')}</span>
          <span className="text-[10px] text-muted-foreground/60">
            · <ImageIcon className="inline h-2.5 w-2.5" /> {t('rigoriumConfig.panels.models.supportsImageInput')}
          </span>
          <button
            type="button"
              ...
              return (
                ...
                >
                  <button
                    ...
                    title={on ? t('rigoriumConfig.panels.models.clickDisable') : t('rigoriumConfig.panels.models.clickEnable')}
                  >
          ...
          return (
            <div key={mid} className="mb-1 flex items-center gap-2 rounded-md border border-border bg-muted/40 px-2 py-1 text-[11px]">
              ...
              <button
                ...
                title={t('rigoriumConfig.actions.remove')}
              >
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            placeholder={t('rigoriumConfig.panels.models.customModelPlaceholder')}
      ...
      <div className="px-4 pb-4">
        ...
        >
          ...
          {t('rigoriumConfig.panels.models.providerAdvancedToggle')}
        </button>
        {showProviderAdvanced && (
          <div className="mt-3 space-y-3 divide-y divide-border rounded-md border border-border p-3">
            <FormRow label={t('rigoriumConfig.panels.models.providerRetry.requestMaxRetries.label')} description={t('rigoriumConfig.panels.models.providerRetry.requestMaxRetries.description')}>
              <NumberInput
            ...
            <FormRow label={t('rigoriumConfig.panels.models.providerRetry.streamMaxRetries.label')} description={t('rigoriumConfig.panels.models.providerRetry.streamMaxRetries.description')}>
              <NumberInput
            ...
            <FormRow label={t('rigoriumConfig.panels.models.providerRetry.streamIdleTimeoutMs.label')} description={t('rigoriumConfig.panels.models.providerRetry.streamIdleTimeoutMs.description')}>
              <NumberInput
            ...
            <FormRow label={t('rigoriumConfig.panels.models.providerRetry.baseDelayMs.label')} description={t('rigoriumConfig.panels.models.providerRetry.baseDelayMs.description')}>
              <NumberInput
            ...
            <FormRow label={t('rigoriumConfig.panels.models.providerRetry.maxDelayMs.label')} description={t('rigoriumConfig.panels.models.providerRetry.maxDelayMs.description')}>
              <NumberInput
  ...
  if (!open) {
    return (
      <Button variant="outline" size="sm" onClick={() => setOpen(true)}>
        ...
        {t('rigoriumConfig.panels.models.addProvider')}
      </Button>
    );
  }
  return (
    <div className="space-y-3 rounded-lg border border-border bg-muted/30 p-4">
      <div className="flex items-center justify-between">
        <div className="text-sm font-medium text-foreground">{t('rigoriumConfig.panels.models.addProviderTitle')}</div>
        <Button variant="ghost" size="sm" onClick={() => setOpen(false)}>{t('rigoriumConfig.panels.models.cancel')}</Button>
      </div>
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
        ...
        >
          <div className="font-medium text-foreground">+ {t('rigoriumConfig.panels.models.customProvider')}</div>
          <div className="mt-0.5 text-[10px] text-muted-foreground">{t('rigoriumConfig.panels.models.manualSetup')}</div>
        </button>
      </div>
    </div>
  );
...
function ModelsSection({ config, onChange }: { config: RigoriumConfig; onChange: (next: RigoriumConfig) => void }) {
  const { t } = useTranslation('settings');
  const providers = config.model?.providers ?? {};
  ...
  return (
    <SettingsSection
      title={t('rigoriumConfig.panels.models.title')}
      description={t('rigoriumConfig.panels.models.description')}
    >
      <div className="space-y-3">
        ...
        {ids.length === 0 && (
          <div className="rounded-md border border-dashed border-border px-3 py-6 text-center text-xs text-muted-foreground">
            {t('rigoriumConfig.panels.models.emptyProviders')}
          </div>
        )}
        {ids.map((id) => (
          <ProviderCard
...
function ensureModelRefConfigured<T extends RigoriumConfig>(config: T, ref: string | undefined): T {
...
function ensureModelRefsConfigured<T extends RigoriumConfig>(config: T, refs: Array<string | undefined>): T {
...
function buildModelRefOptions(config: RigoriumConfig): Array<{ value: string; label: string }> {
...
function activeModelCapabilities(config: RigoriumConfig): {
  ref: string;
  providerId: string;
  modelId: string;
...
function AgentsSection({ config, onChange }: { config: RigoriumConfig; onChange: (next: RigoriumConfig) => void }) {
  const { t } = useTranslation('settings');
  const [showAdvanced, setShowAdvanced] = useState(false);
  ...
  const subOptions = [
    { value: 'inherit', label: t('rigoriumConfig.panels.agents.subagents.inherit') },
    ...refOptions,
  ];
  ...
  return (
    <SettingsSection
      title={t('rigoriumConfig.panels.agents.title')}
      description={t('rigoriumConfig.panels.agents.description')}
    >
      <SettingsCard divided>
        <FormRow label={t('rigoriumConfig.panels.agents.mainModel.label')} description={t('rigoriumConfig.panels.agents.mainModel.description')}>
          <Select
            value={mainRef}
            options={mainOptions}
          ...
          <div className="px-4 py-3">
            <div className="rounded-md border border-border/60 bg-muted/30 p-3">
              <div className="mb-2 text-xs font-medium text-foreground">
                {t('rigoriumConfig.panels.agents.capabilities.title')}
              </div>
              <div className="flex flex-wrap items-center gap-3 text-xs">
                <span className="inline-flex items-center gap-1.5">
                  <ImageIcon className="h-3.5 w-3.5" />
                  {t('rigoriumConfig.panels.agents.capabilities.imageInput')}
                </span>
                <label className="inline-flex items-center gap-2">
                  ...
                  <span className={cn(
                    ...
                    {supportsImageEffective ? t('rigoriumConfig.panels.agents.capabilities.enabled') : t('rigoriumConfig.panels.agents.capabilities.disabled')}
                  </span>
                </label>
              </div>
              <p className="mt-2 text-[10px] leading-relaxed text-muted-foreground">
                {userOverrideActive
                  ? t('rigoriumConfig.panels.agents.capabilities.overrideActive')
                  : caps.catalogModel
                    ? (caps.catalogModel.supportsImage ? t('rigoriumConfig.panels.agents.capabilities.catalogSupportsImage') : t('rigoriumConfig.panels.agents.capabilities.catalogTextOnly'))
                    : t('rigoriumConfig.panels.agents.capabilities.noCatalog')}
                {' '}{t('rigoriumConfig.panels.agents.capabilities.imageWarning')}
              </p>
              ...
              <div className="mt-3 border-t border-border/60 pt-3">
                <div className="flex flex-wrap items-center gap-3 text-xs">
                  <span className="inline-flex items-center gap-1.5">
                    ...
                    {t('rigoriumConfig.panels.agents.capabilities.maxOutputTokens')}
                  </span>
                  <input
                    type="number"
                    min={1}
                ...
                <p className="mt-2 text-[10px] leading-relaxed text-muted-foreground">
                  {t('rigoriumConfig.panels.agents.capabilities.maxOutputDescription')}
                </p>
              </div>
              ...
              <div className="mt-3 border-t border-border/60 pt-3">
                <div className="flex flex-wrap items-center gap-3 text-xs">
                  <span className="inline-flex items-center gap-1.5">
                    <Gauge className="h-3.5 w-3.5" />
                    {t('rigoriumConfig.panels.agents.capabilities.maxContextTokens')}
                  </span>
                  <input
                    type="number"
                    min={1}
                ...
                <p className="mt-2 text-[10px] leading-relaxed text-muted-foreground">
                  {t('rigoriumConfig.panels.agents.capabilities.maxContextDescription')}
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="px-4 py-2.5">
          ...
          >
            <ChevronDown className={cn('h-3.5 w-3.5 transition-transform', showAdvanced && 'rotate-180')} />
            {t('rigoriumConfig.panels.agents.advancedToggle')}
          </button>
        </div>

        {showAdvanced && (
          <div className="divide-y divide-border">
            <FormRow label={t('rigoriumConfig.panels.agents.subagents.label')} description={t('rigoriumConfig.panels.agents.subagents.description')}>
              <Select
                value={subDefault}
                options={subOptions}
            ...
            <div className="flex gap-2 px-4 py-3 text-[11px] leading-5 text-muted-foreground">
              ...
              <p>{t('rigoriumConfig.panels.agents.subagents.routerNote')}</p>
            </div>
          </div>
        )}
      </SettingsCard>
...
function CustomEnvSection({ config, onChange }: { config: RigoriumConfig; onChange: (next: RigoriumConfig) => void }) {
  const { t } = useTranslation('settings');
  ...
  return (
    <SettingsSection
      title={t('rigoriumConfig.panels.customEnv.title')}
      description={t('rigoriumConfig.panels.customEnv.description')}
    >
      <SettingsCard className="space-y-3 p-4">
        {entries.length === 0 && (
          <div className="rounded-md border border-dashed border-border px-3 py-6 text-center text-xs text-muted-foreground">
            {t('rigoriumConfig.panels.customEnv.empty')}
          </div>
        )}
          ...
          return (
            <div key={key} className="space-y-1">
              <div className="flex items-center gap-2">
                ...
                <SecretTextInput
                  value={value}
                  placeholder={isMasked ? t('rigoriumConfig.panels.customEnv.existingValueKept') : 'value'}
                  monospace
              ...
              {isMasked && (
                <div className="ml-[216px] flex items-center gap-1 text-[11px] text-muted-foreground">
                  <Info className="h-3 w-3" />
                  {t('rigoriumConfig.panels.customEnv.valueHidden')}
                </div>
              )}
            </div>
          );
        ...
        <div className="border-t border-border pt-3">
          <div className="mb-2 text-xs font-medium text-foreground">{t('rigoriumConfig.panels.customEnv.addVariable')}</div>
          <div className="flex items-center gap-2">
            <input
              value={newKey}
            ...
            <Button variant="outline" size="sm" className="shrink-0" onClick={addEntry} disabled={!newKey.trim()}>
              <Plus className="mr-1 h-3.5 w-3.5" />
              {t('rigoriumConfig.panels.customEnv.add')}
            </Button>
          </div>
        </div>
        ...
        {unusedWellKnown.length > 0 && (
          <div className="border-t border-border pt-3">
            <div className="mb-2 text-xs text-muted-foreground">{t('rigoriumConfig.panels.customEnv.quickAddKeys')}</div>
...
function AlwaysOnSection({
  ...
  onChange: (next: RigoriumConfig) => void;
}) {
  const { t } = useTranslation('settings');
  ...
  return (
    <SettingsSection
      title={t('rigoriumConfig.panels.alwaysOn.title')}
      description={t('rigoriumConfig.panels.alwaysOn.description')}
    >
      <div className="space-y-4">
        {/* General */}
        <SettingsCard>
          <SettingsRow
            label={t('rigoriumConfig.panels.alwaysOn.enabled.label')}
            description={t('rigoriumConfig.panels.alwaysOn.enabled.description')}
          >
            <SettingsToggle
              checked={enabled}
              ariaLabel={t('rigoriumConfig.panels.alwaysOn.enabled.label')}
        ...
        {enabled && (
          <>
            {/* Trigger */}
            <SettingsSection
              title={t('rigoriumConfig.panels.alwaysOn.trigger.title')}
              description={t('rigoriumConfig.panels.alwaysOn.trigger.description')}
            >
              <SettingsCard divided>
                <SettingsRow
                  label={t('rigoriumConfig.panels.alwaysOn.trigger.autoDiscovery.label')}
                  description={t('rigoriumConfig.panels.alwaysOn.trigger.autoDiscovery.description')}
                >
                  <SettingsToggle
                    checked={trigger.enabled === true}
                    ariaLabel={t('rigoriumConfig.panels.alwaysOn.trigger.autoDiscovery.label')}
                    onChange={(value) => onChange(patch(config, ['alwaysOn', 'trigger', 'enabled'], value))}
                  />
                </SettingsRow>
                <FormRow label={t('rigoriumConfig.panels.alwaysOn.trigger.tickInterval.label')} description={t('rigoriumConfig.panels.alwaysOn.trigger.tickInterval.description')}>
                  <NumberInput
                ...
                <FormRow label={t('rigoriumConfig.panels.alwaysOn.trigger.cooldown.label')} description={t('rigoriumConfig.panels.alwaysOn.trigger.cooldown.description')}>
                  <NumberInput
                ...
                <FormRow label={t('rigoriumConfig.panels.alwaysOn.trigger.dailyBudget.label')} description={t('rigoriumConfig.panels.alwaysOn.trigger.dailyBudget.description')}>
                  <NumberInput
                ...
                <FormRow label={t('rigoriumConfig.panels.alwaysOn.trigger.heartbeatStale.label')} description={t('rigoriumConfig.panels.alwaysOn.trigger.heartbeatStale.description')}>
                  <NumberInput
                ...
                <FormRow label={t('rigoriumConfig.panels.alwaysOn.trigger.recentUserMsg.label')} description={t('rigoriumConfig.panels.alwaysOn.trigger.recentUserMsg.description')}>
                  <NumberInput
                ...
                <FormRow label={t('rigoriumConfig.panels.alwaysOn.trigger.preferChannel.label')} description={t('rigoriumConfig.panels.alwaysOn.trigger.preferChannel.description')}>
                  <Select
            ...
            {/* Dormancy */}
            <SettingsSection
              title={t('rigoriumConfig.panels.alwaysOn.dormancy.title')}
              description={t('rigoriumConfig.panels.alwaysOn.dormancy.description')}
            >
              <SettingsCard divided>
                <SettingsRow
                  label={t('rigoriumConfig.panels.alwaysOn.dormancy.enabled.label')}
                  description={t('rigoriumConfig.panels.alwaysOn.dormancy.enabled.description')}
                >
                  <SettingsToggle
                    checked={dormancy.enabled !== false}
                    ariaLabel={t('rigoriumConfig.panels.alwaysOn.dormancy.enabled.label')}
                    onChange={(value) => onChange(patch(config, ['alwaysOn', 'dormancy', 'enabled'], value))}
                  />
                </SettingsRow>
                <FormRow label={t('rigoriumConfig.panels.alwaysOn.dormancy.debounce.label')} description={t('rigoriumConfig.panels.alwaysOn.dormancy.debounce.description')}>
                  <NumberInput
                ...
                <FormRow label={t('rigoriumConfig.panels.alwaysOn.dormancy.ignoreGlobs.label')} description={t('rigoriumConfig.panels.alwaysOn.dormancy.ignoreGlobs.description')}>
                  <textarea
            ...
            {/* Workspace */}
            <SettingsSection
              title={t('rigoriumConfig.panels.alwaysOn.workspace.title')}
              description={t('rigoriumConfig.panels.alwaysOn.workspace.description')}
            >
              <SettingsCard divided>
                <FormRow label={t('rigoriumConfig.panels.alwaysOn.workspace.gitWorktree.label')} description={t('rigoriumConfig.panels.alwaysOn.workspace.gitWorktree.description')}>
                  <TextInput
                ...
                <FormRow label={t('rigoriumConfig.panels.alwaysOn.workspace.snapshotDir.label')} description={t('rigoriumConfig.panels.alwaysOn.workspace.snapshotDir.description')}>
                  <TextInput
                ...
                <FormRow label={t('rigoriumConfig.panels.alwaysOn.workspace.snapshotMaxBytes.label')} description={t('rigoriumConfig.panels.alwaysOn.workspace.snapshotMaxBytes.description')}>
                  <NumberInput
                ...
                <FormRow label={t('rigoriumConfig.panels.alwaysOn.workspace.maxPlansPerCycle.label')} description={t('rigoriumConfig.panels.alwaysOn.workspace.maxPlansPerCycle.description')}>
                  <NumberInput
                ...
                <SettingsRow
                  label={t('rigoriumConfig.panels.alwaysOn.workspace.gitLfs.label')}
                  description={t('rigoriumConfig.panels.alwaysOn.workspace.gitLfs.description')}
                >
                  <SettingsToggle
                    checked={workspace.gitLfs === true}
                    ariaLabel={t('rigoriumConfig.panels.alwaysOn.workspace.gitLfs.label')}
            ...
            {/* Execution */}
            <SettingsSection
              title={t('rigoriumConfig.panels.alwaysOn.execution.title')}
              description={t('rigoriumConfig.panels.alwaysOn.execution.description')}
            >
              <SettingsCard divided>
                <FormRow label={t('rigoriumConfig.panels.alwaysOn.execution.maxTurns.label')} description={t('rigoriumConfig.panels.alwaysOn.execution.maxTurns.description')}>
                  <NumberInput
                ...
                <FormRow label={t('rigoriumConfig.panels.alwaysOn.execution.maxToolCalls.label')} description={t('rigoriumConfig.panels.alwaysOn.execution.maxToolCalls.description')}>
                  <NumberInput
                ...
                <FormRow label={t('rigoriumConfig.panels.alwaysOn.execution.timeout.label')} description={t('rigoriumConfig.panels.alwaysOn.execution.timeout.description')}>
                  <NumberInput
            ...
            {/* Workspace opt-in */}
            <SettingsSection
              title={t('rigoriumConfig.panels.alwaysOn.workspaceOptIn.title')}
              description={t('rigoriumConfig.panels.alwaysOn.workspaceOptIn.description')}
            >
              <SettingsCard divided>
                {projectRows.length === 0 ? (
                  <div className="px-4 py-6 text-sm text-muted-foreground">
                    {t('rigoriumConfig.panels.alwaysOn.workspaceOptIn.empty')}
                  </div>
                ) : (
...
function CronSection({ config, onChange }: { config: RigoriumConfig; onChange: (next: RigoriumConfig) => void }) {
  const { t } = useTranslation('settings');
  ...
  return (
    <SettingsSection
      title={t('rigoriumConfig.panels.cron.title')}
      description={t('rigoriumConfig.panels.cron.description')}
    >
      <SettingsCard divided>
        <SettingsRow
          label={t('rigoriumConfig.panels.cron.enabled.label')}
          description={t('rigoriumConfig.panels.cron.enabled.description')}
        >
          <SettingsToggle
            checked={enabled}
            ariaLabel={t('rigoriumConfig.panels.cron.enabled.label')}
            onChange={(value) => onChange(patch(config, ['cron', 'enabled'], value))}
          />
        </SettingsRow>
        <FormRow
          label={t('rigoriumConfig.panels.cron.timezone.label')}
          description={t('rigoriumConfig.panels.cron.timezone.description')}
        >
          <TextInput
            value={cron.timezone}
        ...
        <FormRow
          label={t('rigoriumConfig.panels.cron.maxConcurrentRuns.label')}
          description={t('rigoriumConfig.panels.cron.maxConcurrentRuns.description')}
        >
          <NumberInput
...
function MemorySection({
  ...
  const refOptions = buildModelRefOptions(config);
  const options = [
    { value: 'inherit', label: t('rigoriumConfig.panels.memory.model.inherit') },
    ...refOptions,
  ];
  ...
  const projectTargets = useMemo(() => {
    const seen = new Set<string>();
    const fallback = t('rigoriumConfig.panels.memory.data.target.projectFallback');
  ...
  const memoryTargetOptions = useMemo(
    () => [
      ...
      { value: MEMORY_ALL_TARGET, label: t('rigoriumConfig.panels.memory.data.target.all') },
    ],
    [projectTargets, t],
  );
  ...
  const selectedTargetLabel = targetIsAllMemory
    ? t('rigoriumConfig.panels.memory.data.target.all')
    : selectedProjectTarget?.label ?? t('rigoriumConfig.panels.memory.data.target.projectFallback');
  const actionBusy = memoryAction.kind === 'busy';
  const canManageTarget = targetIsAllMemory || Boolean(selectedProjectPath);

  const setMemoryField = (field: keyof NonNullable<RigoriumConfig['memory']>, value: unknown) => {
  ...
  const handleExportMemory = async () => {
    if (!canManageTarget) {
      setMemoryAction({ kind: 'error', message: t('rigoriumConfig.panels.memory.data.errors.missingProject') });
      return;
    }

    setActionBusy('rigoriumConfig.panels.memory.data.status.exporting');
    try {
      ...
      if (!body) {
        throw new Error(t('rigoriumConfig.panels.memory.data.errors.invalidExport'));
      }
      const prefix = targetIsAllMemory ? 'rigorium-memory-all' : `rigorium-memory-${safeDownloadToken(selectedTargetLabel)}`;
      downloadMemoryText(raw, `${prefix}-${Date.now()}.json`);
      setActionSuccess('rigoriumConfig.panels.memory.data.status.exported');
    } catch (error) {
  ...
  const handleImportMemoryFile = async (file: File | null) => {
    ...
    if (!payload) {
      setMemoryAction({ kind: 'error', message: t('rigoriumConfig.panels.memory.data.errors.invalidImport') });
      return;
    }

    const confirmKey = targetIsAllMemory
      ? 'rigoriumConfig.panels.memory.data.confirm.importAll'
      : 'rigoriumConfig.panels.memory.data.confirm.importProject';
    if (!window.confirm(t(confirmKey, { target: selectedTargetLabel }))) {
      return;
    }

    setActionBusy('rigoriumConfig.panels.memory.data.status.importing');
    try {
      const url = targetIsAllMemory
      ...
      setActionSuccess('rigoriumConfig.panels.memory.data.status.imported', body?.warnings);
    } catch (error) {
      setActionError(error);
    }
  };
  ...
  const handleClearMemory = async () => {
    ...
    const confirmKey = targetIsAllMemory
      ? 'rigoriumConfig.panels.memory.data.confirm.clearAll'
      : 'rigoriumConfig.panels.memory.data.confirm.clearProject';
    if (!window.confirm(t(confirmKey, { target: selectedTargetLabel }))) {
      return;
    }

    setActionBusy('rigoriumConfig.panels.memory.data.status.clearing');
    try {
      ...
      setActionSuccess('rigoriumConfig.panels.memory.data.status.cleared');
    } catch (error) {
      setActionError(error);
    }
  };
  ...
  return (
    <SettingsSection
      title={t('rigoriumConfig.panels.memory.title')}
      description={t('rigoriumConfig.panels.memory.description')}
    >
      <div className="space-y-4">
        <SettingsCard divided>
          <SettingsRow
            label={t('rigoriumConfig.panels.memory.enabled.label')}
            description={t('rigoriumConfig.panels.memory.enabled.description')}
          >
            <SettingsToggle
              checked={Boolean(m.enabled)}
              ariaLabel={t('rigoriumConfig.panels.memory.enabled.label')}
          ...
          {m.enabled && (
            <>
              <FormRow
                label={t('rigoriumConfig.panels.memory.model.label')}
                description={t('rigoriumConfig.panels.memory.model.description')}
              >
                <Select
                  value={selected}
                  options={options}
              ...
              <FormRow
                label={t('rigoriumConfig.panels.memory.automation.autoIndex.label')}
                description={t('rigoriumConfig.panels.memory.automation.autoIndex.description')}
              >
                <NumberInput
              ...
              <FormRow
                label={t('rigoriumConfig.panels.memory.automation.autoDream.label')}
                description={t('rigoriumConfig.panels.memory.automation.autoDream.description')}
              >
                <NumberInput
        ...
        <SettingsSection
          title={t('rigoriumConfig.panels.memory.data.title')}
          description={t('rigoriumConfig.panels.memory.data.description')}
        >
          <SettingsCard divided>
            <FormRow
              label={t('rigoriumConfig.panels.memory.data.target.label')}
              description={t('rigoriumConfig.panels.memory.data.target.description')}
            >
              <Select
            ...
            <div className="px-4 py-3">
              <div className="flex flex-wrap gap-2">
                ...
                >
                  ...
                  {t('rigoriumConfig.panels.memory.data.actions.export')}
                </Button>
                <Button
                  type="button"
                  variant="outline"
                ...
                >
                  ...
                  {t('rigoriumConfig.panels.memory.data.actions.import')}
                </Button>
                <Button
                  type="button"
                  variant="destructive"
                ...
                >
                  ...
                  {t('rigoriumConfig.panels.memory.data.actions.clear')}
                </Button>
              </div>
              <input
                ref={importInputRef}
...
function ToolsSection({ config, onChange }: { config: RigoriumConfig; onChange: (next: RigoriumConfig) => void }) {
  ...
  const setField = (field: 'apiKey' | 'endpoint', value: string) => {
    const trimmed = value;
    const nextWs: NonNullable<RigoriumConfig['tools']>['webSearch'] = { ...ws };
  ...
  const handleTest = async () => {
    ...
    if (!trimmedKey) {
      setTestStatus('error');
      setTestMessage(t('rigoriumConfig.panels.tools.test.needsKey'));
      return;
    }
    ...
    try {
      ...
      if (data.ok) {
        ...
        setTestMessage(
          t('rigoriumConfig.panels.tools.test.success', {
        ...
        setTestStatus('error');
        setTestMessage(
          t('rigoriumConfig.panels.tools.test.failedPrefix', { error: data.error || 'unknown' }),
        );
      }
    } catch (err) {
      setTestStatus('error');
      setTestMessage(
        t('rigoriumConfig.panels.tools.test.failedPrefix', {
  ...
  return (
    <SettingsSection
      title={t('rigoriumConfig.panels.tools.title')}
      description={t('rigoriumConfig.panels.tools.description')}
    >
      <SettingsCard divided>
        <FormRow
          label={t('rigoriumConfig.panels.tools.provider.label')}
          description={t('rigoriumConfig.panels.tools.provider.description')}
        >
          <Select
            value={provider}
            options={[
              { value: 'glm', label: t('rigoriumConfig.panels.tools.provider.glm') },
              { value: 'tavily', label: t('rigoriumConfig.panels.tools.provider.tavily') },
              { value: 'custom', label: t('rigoriumConfig.panels.tools.provider.custom') },
            ]}
            onChange={(v) => setProvider(v === 'custom' ? 'custom' : v === 'tavily' ? 'tavily' : 'glm')}
          />
        </FormRow>
        <FormRow
          label={t('rigoriumConfig.panels.tools.apiKey.label')}
          description={t('rigoriumConfig.panels.tools.apiKey.description')}
        >
          <SecretTextInput
            value={apiKey}
            emptyPlaceholder={t('rigoriumConfig.panels.tools.apiKey.placeholder')}
            maskedPlaceholder={t('rigoriumConfig.panels.tools.apiKey.maskedPlaceholder')}
            monospace
          ...
          {isMaskedSecret(apiKey) && (
            <p className="mt-1 flex items-center gap-1 text-[11px] text-muted-foreground">
              <Info className="h-3 w-3" />
              {t('rigoriumConfig.panels.tools.apiKey.keyHidden')}
            </p>
          )}
        </FormRow>
        <FormRow
          label={t('rigoriumConfig.panels.tools.endpoint.label')}
          description={t('rigoriumConfig.panels.tools.endpoint.description')}
        >
          <TextInput
            value={endpointValue}
        ...
        {provider === 'custom' && (
          <>
            <FormRow
              label={t('rigoriumConfig.panels.tools.custom.name.label')}
              description={t('rigoriumConfig.panels.tools.custom.name.description')}
            >
              <TextInput
                value={custom.name ?? ''}
            ...
            <FormRow
              label={t('rigoriumConfig.panels.tools.custom.auth.label')}
              description={t('rigoriumConfig.panels.tools.custom.auth.description')}
            >
              <Select
                value={custom.auth ?? 'bearer'}
                options={[
                  { value: 'bearer', label: t('rigoriumConfig.panels.tools.custom.auth.bearer') },
                  { value: 'bodyApiKey', label: t('rigoriumConfig.panels.tools.custom.auth.bodyApiKey') },
                  { value: 'queryApiKey', label: t('rigoriumConfig.panels.tools.custom.auth.queryApiKey') },
                  { value: 'none', label: t('rigoriumConfig.panels.tools.custom.auth.none') },
                ]}
                onChange={(v) => setCustomField('auth', v)}
              />
            </FormRow>
            <FormRow
              label={t('rigoriumConfig.panels.tools.custom.method.label')}
              description={t('rigoriumConfig.panels.tools.custom.method.description')}
            >
              <Select
            ...
            <FormRow
              label={t('rigoriumConfig.panels.tools.custom.params.label')}
              description={t('rigoriumConfig.panels.tools.custom.params.description')}
            >
            ...
            <FormRow
              label={t('rigoriumConfig.panels.tools.custom.mapping.label')}
              description={t('rigoriumConfig.panels.tools.custom.mapping.description')}
            >
              <div className="grid gap-2 md:grid-cols-2">
        ...
        <div className="flex flex-col gap-2 px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {testStatus === 'testing'
                ? t('rigoriumConfig.panels.tools.test.testing')
                : t('rigoriumConfig.panels.tools.test.button')}
            </Button>
            {testStatus === 'success' && (
...
function DeepSeekNativeSearchSection({ config, onChange }: { config: RigoriumConfig; onChange: (next: RigoriumConfig) => void }) {
  const { t } = useTranslation('settings');
  ...
  const setField = (field: 'apiKey' | 'endpoint' | 'model', value: string) => {
    const nextNative: NonNullable<NonNullable<RigoriumConfig['tools']>['deepseekNativeSearch']> = { ...native };
    if (value === '') {
      delete nextNative[field];
    } else {
      nextNative[field] = value;
    }
    const nextTools: NonNullable<RigoriumConfig['tools']> = { ...(config.tools ?? {}) };
  ...
  const handleTest = async () => {
    ...
    try {
      ...
      if (data.ok) {
        setTestStatus('success');
        setTestMessage(t('rigoriumConfig.panels.deepseekNativeSearch.test.success', {
          count: data.citationCount ?? 0,
          latency: data.latencyMs ?? 0,
        }));
      } else {
        setTestStatus('error');
        setTestMessage(t('rigoriumConfig.panels.deepseekNativeSearch.test.failedPrefix', { error: data.error || 'unknown' }));
      }
    } catch (err) {
      setTestStatus('error');
      setTestMessage(t('rigoriumConfig.panels.deepseekNativeSearch.test.failedPrefix', {
        error: err instanceof Error ? err.message : String(err),
      }));
    }
  };
  ...
  return (
    <SettingsSection
      title={t('rigoriumConfig.panels.deepseekNativeSearch.title')}
      description={t('rigoriumConfig.panels.deepseekNativeSearch.description')}
    >
      <SettingsCard divided>
        <FormRow
          label={t('rigoriumConfig.panels.deepseekNativeSearch.apiKey.label')}
          description={t('rigoriumConfig.panels.deepseekNativeSearch.apiKey.description')}
        >
          <SecretTextInput
            value={apiKey}
            emptyPlaceholder={t('rigoriumConfig.panels.deepseekNativeSearch.apiKey.placeholder')}
            maskedPlaceholder={t('rigoriumConfig.panels.deepseekNativeSearch.apiKey.maskedPlaceholder')}
            monospace
          ...
          {isMaskedSecret(apiKey) && (
            <p className="mt-1 flex items-center gap-1 text-[11px] text-muted-foreground">
              <Info className="h-3 w-3" />
              {t('rigoriumConfig.panels.deepseekNativeSearch.apiKey.keyHidden')}
            </p>
          )}
          {!hasDedicatedKey && reusableProviderId && (
            <p className="mt-1 flex items-center gap-1 text-[11px] text-muted-foreground">
              <CheckCircle2 className="h-3 w-3" />
              {t('rigoriumConfig.panels.deepseekNativeSearch.apiKey.reuseProvider', { provider: reusableProviderId })}
            </p>
          )}
        </FormRow>
        <FormRow
          label={t('rigoriumConfig.panels.deepseekNativeSearch.endpoint.label')}
          description={t('rigoriumConfig.panels.deepseekNativeSearch.endpoint.description')}
        >
          <TextInput
            value={endpointValue}
        ...
        <FormRow
          label={t('rigoriumConfig.panels.deepseekNativeSearch.model.label')}
          description={t('rigoriumConfig.panels.deepseekNativeSearch.model.description')}
        >
          <TextInput
            value={modelValue}
        ...
        <div className="flex flex-col gap-2 px-4 py-3">
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              ...
              {testStatus === 'testing'
                ? t('rigoriumConfig.panels.deepseekNativeSearch.test.testing')
                : t('rigoriumConfig.panels.deepseekNativeSearch.test.button')}
            </Button>
            {testStatus === 'success' && (
...
function ModelPricingEditor({ config, onChange }: { config: RigoriumConfig; onChange: (next: RigoriumConfig) => void }) {
  const { t } = useTranslation('settings');
  ...
  return (
    <SettingsCard className="space-y-3 p-4">
      <div>
        <div className="text-sm font-semibold text-foreground">{t('rigoriumConfig.panels.router.pricing.title')}</div>
        <div className="mt-0.5 text-xs text-muted-foreground">
          {t('rigoriumConfig.panels.router.pricing.description')}
        </div>
      </div>

      {keys.length === 0 && (
        <div className="rounded-md border border-dashed border-border px-3 py-4 text-center text-xs text-muted-foreground">
          {t('rigoriumConfig.panels.router.pricing.empty')}
        </div>
      )}
        ...
        return (
          <div key={key} className="space-y-2 rounded-lg border border-border bg-background/50 p-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              <label className="text-xs text-muted-foreground">
                <span className="mb-1 block">{t('rigoriumConfig.panels.router.pricing.inputPerM')}</span>
                <NumberInput value={entry.input} placeholder="0.50" onChange={(v) => setPricing(key, 'input', v)} />
              </label>
              <label className="text-xs text-muted-foreground">
                <span className="mb-1 block">{t('rigoriumConfig.panels.router.pricing.outputPerM')}</span>
                <NumberInput value={entry.output} placeholder="1.50" onChange={(v) => setPricing(key, 'output', v)} />
              </label>
              <label className="text-xs text-muted-foreground">
                <span className="mb-1 block">{t('rigoriumConfig.panels.router.pricing.cachePerM')}</span>
      ...
      <div className="border-t border-border pt-3">
        <div className="mb-2 text-xs font-medium text-foreground">{t('rigoriumConfig.panels.router.pricing.addTitle')}</div>
        <div className="flex items-center gap-2">
          <input
            value={newKey}
          ...
          <Button variant="outline" size="sm" className="shrink-0" onClick={addPricing} disabled={!newKey.trim()}>
            <Plus className="mr-1 h-3.5 w-3.5" />
            {t('rigoriumConfig.panels.router.pricing.add')}
          </Button>
        </div>
      </div>
    </SettingsCard>
...
function RouterFallbackEditor({ config, onChange }: { config: RigoriumConfig; onChange: (next: RigoriumConfig) => void }) {
  const { t } = useTranslation('settings');
  const fallback = config.router?.fallback ?? {};
  ...
  return (
    <SettingsCard className="space-y-3 p-4">
      <div>
        <div className="text-sm font-semibold text-foreground">{t('rigoriumConfig.panels.router.fallback.title')}</div>
        <div className="mt-0.5 text-xs text-muted-foreground">
          {t('rigoriumConfig.panels.router.fallback.description')}
        </div>
      </div>
      {entries.length === 0 && (
        <div className="rounded-md border border-dashed border-border px-3 py-4 text-center text-xs text-muted-foreground">
          {t('rigoriumConfig.panels.router.fallback.empty')}
        </div>
      )}
      {entries.map(([scenario, chain]) => (
        <div key={scenario} className="space-y-2 rounded-lg border border-border bg-background/50 p-3">
          ...
          <div className="space-y-1.5">
            {(chain ?? []).map((model, idx) => (
              <div key={idx} className="flex items-center gap-2">
                ...
                <button
                  ...
                  title={t('rigoriumConfig.actions.removeModel')}
                >
                  <Trash2 className="h-3 w-3" />
                </button>
              </div>
            ...
            >
              <Plus className="h-3 w-3" />
              {t('rigoriumConfig.panels.router.fallback.addModel')}
            </button>
          </div>
        </div>
      ))}
      <div className="border-t border-border pt-3">
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" className="shrink-0" onClick={addChain} disabled={!newKey.trim()}>
            <Plus className="mr-1 h-3.5 w-3.5" />
            {t('rigoriumConfig.panels.router.fallback.add')}
          </Button>
        </div>
      </div>
    </SettingsCard>
...
function TokenSaverTierEditor({ config, onChange }: { config: RigoriumConfig; onChange: (next: RigoriumConfig) => void }) {
  const { t } = useTranslation('settings');
  ...
  return (
    <div className="space-y-3">
      <div>
        <div className="text-xs font-semibold text-foreground">{t('rigoriumConfig.panels.router.tiers.title')}</div>
        <div className="mt-0.5 text-[11px] text-muted-foreground">
          {t('rigoriumConfig.panels.router.tiers.description')}
        </div>
      </div>
      {entries.map(([key, tier]) => (
        <div key={key} className="space-y-2 rounded-lg border border-border bg-background/50 p-3">
          ...
          <textarea
            ...
            placeholder={t('rigoriumConfig.panels.router.tiers.placeholder')}
            rows={2}
      ...
      <div className="border-t border-border pt-3">
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="sm" className="shrink-0" onClick={addTier} disabled={!newKey.trim()}>
            <Plus className="mr-1 h-3.5 w-3.5" />
            {t('rigoriumConfig.panels.router.tiers.add')}
          </Button>
        </div>
      </div>
    </div>
...
function TokenSaverRulesEditor({ config, onChange }: { config: RigoriumConfig; onChange: (next: RigoriumConfig) => void }) {
  const { t } = useTranslation('settings');
  ...
  return (
    <div className="space-y-2">
      <div className="text-xs font-semibold text-foreground">{t('rigoriumConfig.panels.router.rules.title')}</div>
      {rules.length === 0 && (
        <div className="rounded-md border border-dashed border-border px-3 py-3 text-center text-xs text-muted-foreground">
          {t('rigoriumConfig.panels.router.rules.empty')}
        </div>
      )}
      {rules.map((rule, idx) => (
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          placeholder={t('rigoriumConfig.panels.router.rules.placeholder')}
        ...
        <Button variant="outline" size="sm" className="shrink-0" onClick={addRule} disabled={!newRule.trim()}>
          <Plus className="mr-1 h-3.5 w-3.5" />
          {t('rigoriumConfig.panels.router.rules.add')}
        </Button>
      </div>
    </div>
  );
...
function RouterLevelEditor({ config, onChange }: { config: RigoriumConfig; onChange: (next: RigoriumConfig) => void }) {
  const { t } = useTranslation('settings');
  const modelOpts = buildModelRefOptions(config);
  ...
  return (
    <SettingsCard divided>
      <FormRow
        label={t('rigoriumConfig.panels.router.levels.default.label')}
        description={t('rigoriumConfig.panels.router.levels.default.description')}
      >
        <ModelRefInput
          value={defaultValue}
          options={modelOpts}
          placeholder={t('rigoriumConfig.panels.router.levels.modelPlaceholder')}
          onChange={setDefault}
        />
      </FormRow>

      <FormRow
        label={t('rigoriumConfig.panels.router.levels.judge.label')}
        description={t('rigoriumConfig.panels.router.levels.judge.description')}
      >
        <ModelRefInput
          value={judgeValue}
          options={modelOpts}
      ...
      {ROUTER_TIER_KEYS.map((key) => (
        <FormRow
          key={key}
          label={t(`rigoriumConfig.panels.router.levels.${key}.label`)}
          description={t(`rigoriumConfig.panels.router.levels.${key}.description`)}
        >
          <ModelRefInput
...
function RouterSection({ config, onChange }: { config: RigoriumConfig; onChange: (next: RigoriumConfig) => void }) {
  const { t } = useTranslation('settings');
  const [showAdvanced, setShowAdvanced] = useState(false);
  ...
  const getDefaultModel = (base: RigoriumConfig) =>
  ...
  const seedRouterDefaults = (base: RigoriumConfig) => {
    let next = base;
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <SettingsCard divided>
          <SettingsRow
            label={t('rigoriumConfig.panels.router.enabled.label')}
            description={t('rigoriumConfig.panels.router.enabled.description')}
          >
            <SettingsToggle
              checked={enabled}
              ariaLabel={t('rigoriumConfig.panels.router.enabled.label')}
              onChange={(v) => {
        ...
        {enabled && (
          <>
            ...
            >
              <ChevronDown className={cn('h-3.5 w-3.5 transition-transform', showAdvanced && 'rotate-180')} />
              {t('rigoriumConfig.panels.router.advancedToggle')}
            </button>
            ...
            {showAdvanced && (
              <>
                ...
                <SettingsCard divided>
                  <SettingsRow
                    label={t('rigoriumConfig.panels.router.zeroUsageRetry.label')}
                    description={t('rigoriumConfig.panels.router.zeroUsageRetry.description')}
                  >
                    <SettingsToggle
                      checked={zeroUsageEnabled}
                      ariaLabel={t('rigoriumConfig.panels.router.zeroUsageRetry.label')}
                      onChange={(v) => onChange(patch(config, ['router', 'zeroUsageRetry', 'enabled'], v))}
                    />
                  </SettingsRow>
                  {zeroUsageEnabled && (
                    <FormRow label={t('rigoriumConfig.panels.router.zeroUsageRetry.maxAttempts.label')} description={t('rigoriumConfig.panels.router.zeroUsageRetry.maxAttempts.description')}>
                      <NumberInput
                ...
                <SettingsCard className="space-y-4 p-4">
                  <SettingsRow
                    label={t('rigoriumConfig.panels.router.transientRetry.label')}
                    description={t('rigoriumConfig.panels.router.transientRetry.description')}
                  >
                    <SettingsToggle
                  ...
                  {transientRetryEnabled && (
                    <>
                      <FormRow label={t('rigoriumConfig.panels.router.transientRetry.maxAttempts.label')} description={t('rigoriumConfig.panels.router.transientRetry.maxAttempts.description')}>
                        <NumberInput
                      ...
                      <FormRow label={t('rigoriumConfig.panels.router.transientRetry.baseDelayMs.label')} description={t('rigoriumConfig.panels.router.transientRetry.baseDelayMs.description')}>
                        <NumberInput
                      ...
                      <FormRow label={t('rigoriumConfig.panels.router.transientRetry.maxDelayMs.label')} description={t('rigoriumConfig.panels.router.transientRetry.maxDelayMs.description')}>
                        <NumberInput
                          value={tr.maxDelayMs}
                          placeholder="30000"
                ...
                <SettingsCard className="space-y-4 p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-sm font-semibold text-foreground">{t('rigoriumConfig.panels.router.tokenSaver.title')}</div>
                      <div className="mt-0.5 text-xs text-muted-foreground">
                        {t('rigoriumConfig.panels.router.tokenSaver.description')}
                      </div>
                    </div>
                    <SettingsToggle
                      checked={tokenSaverEnabled}
                      ariaLabel={t('rigoriumConfig.panels.router.tokenSaver.title')}
                      onChange={(v) => {
                  ...
                  {tokenSaverEnabled && (
                    <div className="space-y-4 border-t border-border pt-4">
                      <div>
                        <label className="mb-1 block text-xs font-medium text-foreground">{t('rigoriumConfig.panels.router.tokenSaver.defaultTier')}</label>
                        <Select
                      ...
                      <FormRow label={t('rigoriumConfig.panels.router.tokenSaver.judgeTimeout.label')} description={t('rigoriumConfig.panels.router.tokenSaver.judgeTimeout.description')}>
                        <NumberInput
                      ...
                      <FormRow label={t('rigoriumConfig.panels.router.tokenSaver.subagentPolicy.label')} description={t('rigoriumConfig.panels.router.tokenSaver.subagentPolicy.description')}>
                        <Select
                ...
                <SettingsCard className="space-y-4 p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="text-sm font-semibold text-foreground">{t('rigoriumConfig.panels.router.autoOrchestrate.title')}</div>
                      <div className="mt-0.5 text-xs text-muted-foreground">
                        {t('rigoriumConfig.panels.router.autoOrchestrate.description')}
                      </div>
                    </div>
                    <SettingsToggle
                      checked={autoOrchestrateEnabled}
                      ariaLabel={t('rigoriumConfig.panels.router.autoOrchestrate.title')}
                  ...
                  {autoOrchestrateEnabled && (
                    <div className="space-y-3 border-t border-border pt-4">
                      <div>
                        <label className="mb-1 block text-xs font-medium text-foreground">{t('rigoriumConfig.panels.router.autoOrchestrate.triggerTiers')}</label>
                      ...
                      <SettingsRow
                        label={t('rigoriumConfig.panels.router.autoOrchestrate.slimPrompt.label')}
                        description={t('rigoriumConfig.panels.router.autoOrchestrate.slimPrompt.description')}
                      >
                        <SettingsToggle
                          checked={ao.slimSystemPrompt !== false}
                          ariaLabel={t('rigoriumConfig.panels.router.autoOrchestrate.slimPrompt.label')}
                ...
                <SettingsCard divided>
                  <SettingsRow
                    label={t('rigoriumConfig.panels.router.stats.label')}
                    description={t('rigoriumConfig.panels.router.stats.description')}
                  >
                    <SettingsToggle
                      checked={statsEnabled}
                      ariaLabel={t('rigoriumConfig.panels.router.stats.label')}
...
function GatewaySection({ config, onChange }: { config: RigoriumConfig; onChange: (next: RigoriumConfig) => void }) {
  const { t } = useTranslation('settings');
  const g = config.gateway ?? {};
  return (
    <SettingsSection
      title={t('rigoriumConfig.panels.gateway.title')}
      description={t('rigoriumConfig.panels.gateway.description')}
    >
      <SettingsCard divided>
        <SettingsRow label={t('rigoriumConfig.panels.gateway.enabled.label')} description={t('rigoriumConfig.panels.gateway.enabled.description')}>
          <SettingsToggle
            checked={Boolean(g.enabled)}
            ariaLabel={t('rigoriumConfig.panels.gateway.enabled.label')}
            onChange={(v) => onChange(patch(config, ['gateway', 'enabled'], v))}
          />
        </SettingsRow>
        {g.enabled && (
          <FormRow label={t('rigoriumConfig.panels.gateway.home.label')} description={t('rigoriumConfig.panels.gateway.home.description')}>
            <TextInput value={g.home} placeholder="~/.rigorium/gateway" monospace onChange={(v) => onChange(patch(config, ['gateway', 'home'], v))} />
          </FormRow>
        )}
      </SettingsCard>
    </SettingsSection>
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="text-[15px] font-semibold leading-5 text-foreground">
          {t(`rigoriumConfig.sections.${meta.labelKey}.label`)}
        </div>
        <div className="mt-0.5 text-xs leading-5 text-muted-foreground">
          {t(`rigoriumConfig.sections.${meta.descriptionKey}.description`)}
        </div>
      </div>
  ...
  return (
    <div className="space-y-8">
      {SECTION_GROUPS.map((group) => (
        <ConfigSectionGroup
          key={group.id}
          title={t(`rigoriumConfig.sectionGroups.${group.id}`)}
          description={group.id === 'basic' ? t('rigoriumConfig.sectionGroups.basicDescription') : undefined}
        >
...
export default function RigoriumConfigTab({
  projects = [],
  initialSection,
}: {
  projects?: SettingsProject[];
  ...
  const onFormChange = (next: RigoriumConfig) => {
    try {
  ...
  const configStatusLabel = configHasErrors
    ? t('rigoriumConfig.rawYaml.configInvalid')
    : isDirty
      ? t('rigoriumConfig.status.unsavedChanges')
      : t('rigoriumConfig.status.noUnsavedChanges');
  const revealFileLabel =
    typeof navigator !== 'undefined' && /win/i.test(navigator.platform)
      ? t('rigoriumConfig.actions.revealFileWindows')
      : typeof navigator !== 'undefined' && /mac/i.test(navigator.platform)
        ? t('rigoriumConfig.actions.revealFileMac')
        : t('rigoriumConfig.actions.revealFileGeneric');
    ...
    <div className="space-y-6">
      <SettingsCard className="p-3">
        <div className="flex flex-col gap-3 md:flex-row md:items-center">
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <div className="text-[15px] font-semibold leading-5 text-foreground">
                {exists ? t('rigoriumConfig.header.configFile') : t('rigoriumConfig.header.configPreview')}
              </div>
          ...
          <div className="flex flex-wrap items-center gap-2 md:justify-end">
            ...
            <Button type="button" size="sm" onClick={save} disabled={saving || !isDirty} className="h-8 gap-1.5 px-2.5 text-xs">
              ...
              {saving ? t('rigoriumConfig.actions.saving') : t('rigoriumConfig.actions.saveAndReloadShort')}
            </Button>
            <button
              type="button"
              onClick={() => setShowConfigDetails((next) => !next)}
              className="inline-flex h-8 w-8 items-center justify-center rounded-md text-muted-foreground transition-colors hover:bg-accent hover:text-foreground"
              aria-label={showConfigDetails ? t('rigoriumConfig.actions.hideDetails') : t('rigoriumConfig.actions.showDetails')}
              title={showConfigDetails ? t('rigoriumConfig.actions.hideDetails') : t('rigoriumConfig.actions.showDetails')}
            >
          ...
          <>
            <div className="mt-3 space-y-3 border-t border-border pt-3">
              <div className="flex flex-wrap items-center gap-2">
                <Button variant="outline" size="sm" onClick={openFile} disabled={opening} className="h-8 gap-1.5 px-2.5 text-xs">
                  ...
                  {opening ? t('rigoriumConfig.actions.opening') : revealFileLabel}
                </Button>
                ...
                <Button type="button" variant="outline" size="sm" onClick={reloadConfig} disabled={saving} className="h-8 gap-1.5 px-2.5 text-xs">
                  <RefreshCw className="mr-1.5 h-3.5 w-3.5" />
                  {t('rigoriumConfig.actions.reloadCurrent')}
                </Button>
              </div>
            ...
            {validation?.valid ? (
              <div className="rounded-lg bg-muted/30 px-3.5 py-2.5">
                <div className="flex items-center gap-2 text-[13px] font-semibold leading-5 text-foreground">
                  ...
                  {t('rigoriumConfig.rawYaml.configValid')}
                </div>
                <div className="mt-0.5 pl-4 text-[11px] leading-4 text-muted-foreground">
                  {isDirty ? t('rigoriumConfig.status.unsavedChanges') : t('rigoriumConfig.status.noUnsavedChanges')}
                </div>
              </div>
            ) : (
              <div className="rounded-lg border border-destructive/30 bg-destructive/10 px-3.5 py-2.5 text-destructive">
                ...
                <div className="mt-0.5 pl-6 text-[11px] leading-4">
                  {t('rigoriumConfig.status.fixYamlInRawEditor')}
                </div>
              </div>
            )}
            ...
            {validation && validation.errors.length > 0 && (
              <div className="rounded-lg border border-destructive/30 bg-destructive/10 p-3 text-xs text-destructive">
                <div className="mb-1 font-semibold">{t('rigoriumConfig.rawYaml.errors')}</div>
            ...
            {validation && validation.warnings.length > 0 && (
              <div className="rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-700 dark:text-amber-300">
                <div className="mb-1 font-semibold">{t('rigoriumConfig.rawYaml.warnings')}</div>
                <ul className="list-disc space-y-1 pl-4">
            ...
            {externalChangeNotice && (
              <div className="mt-3 flex items-start justify-between gap-3 rounded-lg border border-amber-500/40 bg-amber-500/10 p-3 text-xs text-amber-700 dark:text-amber-300">
                ...
                >
                  {t('rigoriumConfig.actions.dismiss')}
                </button>
              </div>
            )}
            {parseError && (
              <div className="mt-3 rounded-lg border border-destructive/30 bg-destructive/10 p-3 text-xs text-destructive">
                {serverParseError
                  ? t('rigoriumConfig.rawYaml.yamlParseErrorWithDetail', { error: serverParseError })
                  : t('rigoriumConfig.rawYaml.yamlParseError')}
              </div>
            )}
          </>
        )}
      ...
      {!configDisabled && parsedConfig ? (
        activeSection ? (
          <div className="space-y-4">
            ...
            >
              ...
              {t('rigoriumConfig.sections.backToMenu')}
            </button>
        ...
        <SettingsCard className="space-y-3 p-5">
          <div>
            <div className="text-sm font-semibold text-foreground">{t('rigoriumConfig.rawYaml.rawYaml')}</div>
            <div className="mt-1 text-xs text-muted-foreground">{t('rigoriumConfig.rawYaml.cannotParse')}</div>
          </div>
          <textarea
            value={raw}
