...
import { buildRigoriumToolPermissionEntry, formatToolInputForDisplay } from '../../utils/chatPermissions';
import { getRigoriumSettings } from '../../utils/chatStorage';
...
export default function PermissionRequestsBanner({
  ...
  for (const request of pendingPermissionRequests) {
    ...
    const entry = buildRigoriumToolPermissionEntry(request.toolName, rawInput) ?? request.requestId;
  ...
  return (
    <div className="mb-3 space-y-2">
      ...
      {Array.from(grouped.entries()).map(([entry, requests]) => {
        ...
        const permissionEntry = buildRigoriumToolPermissionEntry(first.toolName, rawInput);
