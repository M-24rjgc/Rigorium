...
import type { ChatMessage, RigoriumPermissionSuggestion, PermissionGrantResult } from '../types/types.js';
import {
...
export function buildRigoriumToolPermissionEntry(toolName?: string, toolInput?: unknown) {
...
// Backend `RigoriumToolErrorCode` values that map to "user can fix this by
...
export function getRigoriumPermissionSuggestion(
  message: ChatMessage | null | undefined,
  _provider: string,
): RigoriumPermissionSuggestion | null {
  ...
  const entry = buildRigoriumToolPermissionEntry(toolName, message.toolInput);
...
export function grantRigoriumToolPermission(entry: string | null): PermissionGrantResult {
  ...
  safeLocalStorage.setItem(RIGORIUM_SETTINGS_KEY, JSON.stringify(updatedSettings));
  saveRigoriumPermissionSettings({
    allowedTools: nextAllowed,
    disallowedTools: nextDisallowed,
  }).catch((error) => {
