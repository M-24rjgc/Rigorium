---
name: rigorium-skills-migration
description: >-
  Migrate Claude Code, OpenClaw, Hermes, or custom Agent Skill directories into
  Rigorium's global skills directory. Use when the user asks to migrate,
  import, copy, or consolidate skills into Rigorium, or mentions
  ~/.claude/skills, ~/.openclaw, ~/.hermes, ~/.agents/skills, or
  ~/.rigorium/skills.
---

# Rigorium Skills Migration

Use this skill to migrate Agent Skill folders into Rigorium's global skill
store, `~/.rigorium/skills`.
...
The Rigorium migrator scans immediate child directories containing `SKILL.md`
from:
...
- Prefer `--rename` over `--overwrite` unless the user explicitly wants to
  replace existing Rigorium skills.
- The repo bootstrap syncs this skill into `$RIGORIUM_HOME/skills` from
  `skills/rigorium-skills-migration/SKILL.md`; it skips existing targets.
