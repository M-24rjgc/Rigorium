...
const RUN = process.env.RIGORIUM_RUN_LIVE_LLM_CENTER === "1";
if (!RUN) {
  console.log("Skipping live LLM Center smoke tests. Set RIGORIUM_RUN_LIVE_LLM_CENTER=1 to run.");
  process.exit(0);
}

const apiKey = requiredEnv("RIGORIUM_LLM_CENTER_API_KEY");
const baseUrl = stripTrailingSlash(process.env.RIGORIUM_LLM_CENTER_BASE_URL || "https://llm-center.ali.modelbest.cn/llm");
const contextProbeChars = Number.parseInt(process.env.RIGORIUM_LLM_CENTER_CONTEXT_PROBE_CHARS || "150000", 10);

const cases = [
  {
    name: "openai-chat-completions",
    model: process.env.RIGORIUM_LLM_CENTER_OPENAI_MODEL,
  ...
  },
  {
    ...
    model: process.env.RIGORIUM_LLM_CENTER_ANTHROPIC_MODEL,
    path: "/v1/messages",
  ...
  },
  {
    ...
    model: process.env.RIGORIUM_LLM_CENTER_RESPONSES_MODEL,
    path: "/v1/responses",
    body: (model, maxTokens = 65_536, prompt = "Reply with exactly: ok") => ({
      model,
  ...
  },
  {
    ...
    model: process.env.RIGORIUM_LLM_CENTER_GEMINI_MODEL,
...
assert.ok(ran > 0, "No live cases ran; set at least one RIGORIUM_LLM_CENTER_*_MODEL env var.");
...
async function runContextProbe(testCase) {
  if (!Number.isFinite(contextProbeChars) || contextProbeChars <= 0) {
    console.log(`SKIP ${testCase.name} context-cap probe: RIGORIUM_LLM_CENTER_CONTEXT_PROBE_CHARS<=0`);
    return;
  }
