...
import type { RigoriumCommandRunner } from "../../src/tool/builtin/bash/commandRunner.js";
import { ToolRuntime } from "../../src/tool/execution/ToolRuntime.js";
import { ToolRegistry } from "../../src/tool/registry/ToolRegistry.js";
import { toCanonicalToolResultBlock } from "../../src/tool/protocol/result.js";

function createRuntime(runner: RigoriumCommandRunner): ToolRuntime {
...
test("bash failure tool result includes raw stdout and stderr tail for UI and model", async () => {
  const dir = await mkdtemp(join(tmpdir(), "rigorium-bash-error-budget-"));
...
test("bash success keeps full output for ToolResultBudget persistence", async () => {
  const dir = await mkdtemp(join(tmpdir(), "rigorium-bash-budget-"));
