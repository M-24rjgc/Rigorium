...
This document defines the common Rigorium agent workflow for using `todo_write` outside and inside Plan Mode. It is an implementation-facing specification for prompt, tool, and runtime behavior.
...
If a stable session id is not available to the model, use a clear fallback such as `.rigorium/work/current/` and mention it in the final response.
...
- The current workspace is the tool/runtime `cwd`; this is the correct root for `.rigorium/work/...` artifacts.
