...
- **端到端路由对比 harness**（`scripts/benchmark-router-e2e.mjs`）：
  ...
  - 有 key：`RIGORIUM_E2E_BASE_URL + RIGORIUM_E2E_API_KEY`（或直接在
    rigorium.yaml 配置好 provider——harness 自动发现，跳过占位符端点）即跑
    真实端点对比（judge-only vs heuristic+judge 的调用数/延迟 p50/p95/一致性）
