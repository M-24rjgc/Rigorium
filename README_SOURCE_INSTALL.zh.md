...
本文档适合希望直接从源码运行 Rigorium 的开发者。如果只是想快速体验，建议优先使用一键安装脚本或 Docker。
...
| Docker Desktop | 启用 WSL2 backend 的 Docker Desktop、Git for Windows | 不想在本机管理 Node/native build 环境，只想运行 Rigorium |
...
缺少命令说明对应工具还没有安装，或还没有加入 `PATH`。安装工具后，请关闭并重新打开 PowerShell 再检查。Git for Windows 会包含 Git Bash；Rigorium 在 Windows 上会优先使用 Git Bash 作为默认终端 shell，只有找不到 Git Bash 时才回退到 PowerShell。
...
执行上面的前置依赖检查命令时，请使用分开的 PowerShell 命令行，不要使用 Bash 风格的链式命令。安装 Git for Windows 后，Rigorium 内置终端会自动优先使用 Git Bash。如果 PowerShell 拦截 `npm.ps1`，请改用 `npm.cmd`。

#### Portable Node 验证路径

如果想在全局安装 Node.js 前先验证 Rigorium，可只在当前 PowerShell 会话中使用官方 Windows Node.js zip：
...
在 Windows 上，如果 PowerShell 拦截 `npm.ps1`，请使用 `npm.cmd install -g clawhub`。如果使用 Portable Node，`clawhub` 会安装到当前 portable Node 前缀下；运行 Rigorium 时需要继续保留该 Node 目录在 `PATH` 中。

## 首次 Onboarding

Rigorium 当前从 `~/.rigorium/rigorium.yaml` 读取配置。如果本机还没有配置文件，生产模式启动前请先准备 Web UI 的首次 onboarding 流程：

```bash
node scripts/bootstrap-rigorium-config.mjs
```

该命令会初始化 `~/.rigorium/rigorium.yaml`，让 Gateway 可以启动并进入首次 onboarding。随后打开 Web UI，在 onboarding/设置面板中完成 Provider 和 API Key 配置。
...
- 启动时报 `EADDRINUSE`：默认 `3001` 或 `18789` 已被占用，设置 `SERVER_PORT`、`RIGORIUM_GATEWAY_PORT` 和 `RIGORIUM_GATEWAY_URL` 后重试。
- 已有 `~/.rigorium/rigorium.yaml` 但仍进入 onboarding：检查配置里是否仍是 `PLACEHOLDER_RUN_ONBOARDING_TO_REPLACE` 或 `_placeholder/_placeholder`，需要替换为真实 Provider、API Key 和模型。
