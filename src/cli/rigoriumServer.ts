...
import { resolveRigoriumHome, type RigoriumConfig } from "../rigorium/index.js";
import {
...
export type StartRigoriumServerOptions = {
  gateway: Gateway;
  port?: number;
  host?: string;
  ...
  /**
   * Loaded rigorium.yaml config — passed into channel.start() so adapters can
   * read their own section (e.g. `adapters.feishu.appId/appSecret`).
   */
  config?: RigoriumConfig;
};

export type RigoriumServer = GatewayServer & {
  /**
...
export async function startRigoriumServer(options: StartRigoriumServerOptions): Promise<RigoriumServer> {
  const consoleLogger = {
  ...
  const reportChannelStatus = createSafeChannelStatusReporter(
    createChannelRuntimeStatusReporter(resolveRigoriumHome(process.env)),
    consoleLogger,
  );
  const baseDeps = {
