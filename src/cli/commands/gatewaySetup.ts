...
const RIGORIUM_YAML_PATH = process.env.RIGORIUM_CONFIG_PATH || join(RIGORIUM_HOME, "rigorium.yaml");
const WEIXIN_CREDS_PATH = join(RIGORIUM_HOME, "weixin-credentials.json");
...
export async function runGatewaySetup(argv: string[]): Promise<void> {
  ...
  try {
    console.log("\n╔══════════════════════════════════════════════════╗");
    console.log("║  Rigorium Gateway Setup                        ║");
...
async function setupFeishu(rl: ReturnType<typeof createInterface>): Promise<void> {
  ...
  console.log("\n✅ 飞书配置已写入 rigorium.yaml");
  console.log("   连接模式: stream (WebSocket, 推荐 — 无需公网 IP)");
  console.log("   重启 Rigorium 服务后生效\n");
}
...
async function fetchJson(fetchImpl: typeof fetch, url: string): Promise<Record<string, unknown>> {
  const res = await fetchImpl(url, {
    headers: { "User-Agent": "Rigorium/0.1" },
  });
  if (!res.ok) {
...
function saveWeixinCredentials(creds: {
  ...
  mkdirSync(join(homedir(), ".rigorium"), { recursive: true });
...
function loadYamlConfig(): Record<string, any> | null {
  try {
    if (!existsSync(RIGORIUM_YAML_PATH)) return null;
    const raw = readFileSync(RIGORIUM_YAML_PATH, "utf-8");
...
function saveYamlConfig(config: Record<string, any>): void {
  mkdirSync(dirname(RIGORIUM_YAML_PATH), { recursive: true });
  const yamlStr = stringifyYaml(config, {
    lineWidth: 0,
    singleQuote: false,
  });
  writeFileSync(RIGORIUM_YAML_PATH, yamlStr, "utf-8");
}
