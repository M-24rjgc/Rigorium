...
async function main(argv = process.argv.slice(2)): Promise<void> {
  ...
  if (command === "server") {
    ...
    const snapshot = loadRigoriumConfig({ projectRoot, env });
    ...
    // takes precedence; this fills in when only rigorium.yaml has a proxy).
    ...
    const {
      ...
      rigoriumHome,
      env,
      fallbackProjectRoot: rigoriumHome,
    ...
    configStore.subscribe((event) => {
      ...
      if (adapterChanged) {
        reloadChain = reloadChain
          ...
          .catch((err) =>
            console.warn(
              `[rigorium] adapter hot-reload failed: ${err instanceof Error ? err.message : String(err)}`,
            ),
          );
      }
      ...
      reloadChain = reloadChain
        ...
        .catch((err) =>
          console.warn(
            `[rigorium] subsystem reload failed: ${err instanceof Error ? err.message : String(err)}`,
          ),
        );
    });
    ...
    async function handleSubsystemReload(
      ...
      if (cronChanged) parts.push(`cron=${cron ? "started" : "stopped"}`);
        console.log(`[rigorium] Subsystem hot-reload complete: ${parts.join(", ")}`);
    }

    // --- Channel state persistence ---

    const channelStatePersistence = new ChannelStatePersistence({
      stateDir: resolve(rigoriumHome, "channels"),
    });

    // --- Adapter hot-reload ---

    let serverRef: Awaited<ReturnType<typeof startRigoriumServer>> | undefined;
    ...
    async function handleAdapterHotReload(config: (typeof snapshot)["config"]): Promise<void> {
      ...
      if (parts.length) {
        console.log(`[rigorium] Adapter hot-reload complete: ${parts.join(", ")}`);
      }
    }

    // --- Server startup ---

    const envPort = Number.parseInt(env.RIGORIUM_GATEWAY_PORT ?? "", 10);
    ...
    console.log(`Rigorium server listening: ${server.url}`);
  ...
  if (command === "gateway") {
    ...
    console.error("Usage: rigorium gateway setup [feishu|weixin|wecom]");
    process.exitCode = 1;
    return;
  }
...
async function handleCronCommand(argv: string[]): Promise<void> {
  ...
  if (!gateway) {
    console.error("rigorium cron requires a running Rigorium server.");
    process.exitCode = 1;
    return;
  }
  const command = argv[0];
  ...
  if (command === "create") {
    ...
    if (!message || !sessionKey || (!once && !cron)) {
      console.error("Usage: rigorium cron create --session <sessionKey> --message <text> (--once <iso> | --cron <expr>)");
      process.exitCode = 1;
      return;
    }
  ...
  if (command === "delete") {
    ...
    if (!taskId) {
      console.error("Usage: rigorium cron delete <taskId> [--stop-running]");
      process.exitCode = 1;
      return;
    }
  ...
  if (command === "stop") {
    ...
    if (!taskId && !runId) {
      console.error("Usage: rigorium cron stop <taskId> or rigorium cron stop --run <runId>");
      process.exitCode = 1;
      return;
    }
  ...
  console.error("Usage: rigorium cron <list|create|delete|stop>");
  process.exitCode = 1;
}
...
async function handleSkillsCommand(argv: string[]): Promise<void> {
  const command = argv[0];
  if (command !== "migrate") {
    console.error("Usage: rigorium skills migrate [--execute] [--from cc,openclaw,hermes] [--source <dir>] [--overwrite|--rename]");
    process.exitCode = 1;
    return;
  }
  ...
  const report = await migrateSkillsToRigorium({
    rigoriumHome,
    projectRoot,
    include: from,
...
function printSkillMigrationReport(report: Awaited<ReturnType<typeof migrateSkillsToRigorium>>): void {
  const mode = report.mode === "execute" ? "EXECUTED" : "DRY RUN";
  console.log(`Rigorium skills migration (${mode})`);
...
function createFallbackGateway(): Gateway {
  async function* errorStream(input: GatewaySubmitTurnInput): AsyncIterable<GatewayEvent> {
    yield {
      ...
      message: `No Rigorium server is available and local config could not start session ${input.sessionKey}.`,
      recoverable: false,
    };
  }
  return {
