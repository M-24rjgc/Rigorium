...
const DEFAULT_MODEL_NAME = "rigorium-gateway";
  ...
  private async streamTurn(res: ServerResponse, chatId: string, sessionKey: string, message: string): Promise<void> {
    if (!this.gateway) {
      sendJson(res, 503, createApiServerErrorBody({
        ...
        type: "server_error",
        scope: "preflight",
        userHint: "Start or reconnect the Rigorium gateway, then retry.",
      }));
      return;
    }
    ...
    try {
      ...
      try {
        res.write(`data: ${JSON.stringify(createApiServerErrorBody({
          ...
          type: "server_error",
          scope: "channel",
          userHint: "Rigorium failed before this API request could finish. Retry the request; if it repeats, check the API server and gateway logs.",
        }))}\n\n`);
      } catch { /* best effort */ }
    } finally {
