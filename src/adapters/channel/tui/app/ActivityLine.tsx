...
export function ActivityLine({ state }: { state: TuiAppState }): React.ReactNode {
  ...
  return (
    <Box paddingX={1}>
      {state.isRunning ? (
        <Text color={rigoriumDarkBlueTheme.brandAccent}>
          {frame} thinking <Text color={rigoriumDarkBlueTheme.subtle}>· {elapsed}</Text>{" "}
        </Text>
      ) : null}
      {latest.map((item) => (
...
function colorForStatus(status: string): string {
  if (status === "done") return rigoriumDarkBlueTheme.success;
  if (status === "error") return rigoriumDarkBlueTheme.error;
  if (status === "running") return rigoriumDarkBlueTheme.brandAccent;
  return rigoriumDarkBlueTheme.subtle;
}
