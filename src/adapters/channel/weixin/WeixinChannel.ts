...
const CREDENTIALS_PATH = join(homedir(), ".rigorium", "weixin-credentials.json");
...
export class WeixinChannel implements ChannelAdapter {
  ...
  constructor(options: WeixinChannelOptions = {}) {
    ...
    this.attachmentStore = new ImAttachmentStore({
      rootDir: join(homedir(), ".rigorium", "im-attachments"),
  ...
  private startQrLoginInBackground(generation: number): void {
    ...
    console.log("╚══════════════════════════════════════════════╝\n");
    console.log("[weixin] 等待扫码登录；Rigorium Web UI 已可继续使用。\n");
    ...
    try {
      ...
      await liveReply.handleEvent({
        ...
        detail: createVisibleErrorStatusDetail({
          ...
          userHint: "Rigorium failed before this IM turn could finish. Retry the message; if it repeats, check the channel and gateway logs.",
          scope: "channel",
          source: "im_channel",
          detail: {
            channel: "weixin",
  ...
  private saveCredentials(creds: SavedCredentials): void {
    try {
      const dir = join(homedir(), ".rigorium");
