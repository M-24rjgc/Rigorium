...
function resolveUserHint(
  ...
  switch (code) {
    ...
    case "auth_error":
      return {
        userHint: `API key rejected${provider ? ` by provider \"${provider}\"` : " by the provider"}. Update the key in Settings → Model Provider or run rigorium setup.`,
        settingsFix: {
          description: "Reconfigure API key via setup.",
          command: "rigorium setup",
        },
      };
    case "model_not_found":
      return {
        userHint: `The requested model does not exist or your account lacks access${provider ? ` on provider \"${provider}\"` : ""}. Select a valid model in Settings → Model Provider or add it under model.providers.<id>.models in rigorium.yaml.`,
        settingsFix: {
