...
import {
  ...
  type RigoriumMemoryApiType,
  type RigoriumMemoryConfig,
  type RigoriumMemoryReasoningMode,
  type RigoriumMemoryScheduleConfig,
} from "./types.js";
...
): RigoriumMemoryConfig | undefined {
...
function parseMemorySchedule(
  value: unknown,
  diagnostics: RigoriumConfigDiagnostic[],
): RigoriumMemoryScheduleConfig | undefined {
  if (value === undefined) {
    return undefined;
  }
  if (!isRecord(value)) {
    throw new RigoriumConfigError("CONFIG_MEMORY_VALUE_INVALID", "memory.schedule must be an object.");
  }
  const schedule: RigoriumMemoryScheduleConfig = {};
...
function buildScheduleFromFlatFields(rawMemory: Record<string, unknown>): RigoriumMemoryScheduleConfig | undefined {
  const schedule: RigoriumMemoryScheduleConfig = {};
...
function readMemoryApiType(value: unknown): RigoriumMemoryApiType | undefined {
  if (value === undefined) {
    return undefined;
  }
  if (
...
function readOptionalMemoryReasoningMode(value: unknown): RigoriumMemoryReasoningMode | undefined {
  if (value === undefined) {
    return undefined;
  }
...
function readCaptureStrategy(value: unknown): RigoriumMemoryConfig["captureStrategy"] {
  if (value === undefined) {
    return "last_turn";
  }
  ...
  throw new RigoriumConfigError("CONFIG_MEMORY_CAPTURE_INVALID", "memory.captureStrategy must be last_turn or full_session.");
}
...
function readString(value: unknown, fallback: string, path: string): string {
  ...
  if (typeof value !== "string" || value.length === 0) {
    throw new RigoriumConfigError("CONFIG_MEMORY_VALUE_INVALID", `${path} must be a non-empty string.`);
  }
  return value;
}
...
function readBoolean(value: unknown, fallback: boolean, path: string): boolean {
  ...
  if (typeof value !== "boolean") {
    throw new RigoriumConfigError("CONFIG_MEMORY_VALUE_INVALID", `${path} must be a boolean.`);
  }
  return value;
}
...
function readOptionalPositiveNumber(value: unknown, path: string): number | undefined {
  ...
  if (typeof value !== "number" || !Number.isFinite(value) || value <= 0) {
    throw new RigoriumConfigError("CONFIG_MEMORY_VALUE_INVALID", `${path} must be a positive number.`);
  }
  return value;
}
...
function readOptionalNonNegativeInteger(value: unknown, path: string): number | undefined {
  ...
  if (typeof value !== "number" || !Number.isFinite(value) || value < 0) {
    throw new RigoriumConfigError("CONFIG_MEMORY_VALUE_INVALID", `${path} must be a non-negative number.`);
  }
  return Math.floor(value);
}
