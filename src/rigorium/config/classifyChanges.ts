import type { RigoriumConfigChangeClass, RigoriumConfigSnapshot } from "./types.js";

export function diffConfigSnapshots(
  previous: RigoriumConfigSnapshot,
  next: RigoriumConfigSnapshot,
): string[] {
  return diffValues(previous.config, next.config);
}

export function classifyConfigChanges(changedPaths: string[]): RigoriumConfigChangeClass[] {
  const classes = new Set<RigoriumConfigChangeClass>();
...
function classifyPath(path: string): RigoriumConfigChangeClass {
...
function classifyRouterPath(path: string): RigoriumConfigChangeClass {
  if (
