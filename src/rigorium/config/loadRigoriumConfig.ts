...
import { getRigoriumConfigFilePath, getRigoriumMemoryRootDir, resolveRigoriumHome } from "../paths.js";
...
import {
  ...
  type RigoriumTelemetryConfig,
} from "./types.js";

const SUPPORTED_SCHEMA_VERSION = 1;
const ENV_CONFIG_OVERRIDES = [
  ["RIGORIUM_AGENT_MODEL", ["agent", "model"]],
] as const;

export function loadRigoriumConfig(options: RigoriumConfigLoadOptions = {}): RigoriumConfigSnapshot {
  const env = options.env ?? process.env;
  const loadedAt = new Date();
  const diagnostics: RigoriumConfigDiagnostic[] = [];
  const sources: RigoriumConfigSource[] = [];

  const rigoriumHome = resolveRigoriumHome(env);
  if (env.RIGORIUM_HOME) {
    sources.push({
      kind: "env",
      phase: "bootstrap",
      priority: 30,
      loadedAt,
      contentHash: sha256("RIGORIUM_HOME=<redacted-path>"),
    });
  }

  const defaultConfigPath = getRigoriumConfigFilePath(rigoriumHome);
  ...
  const rawConfig = mergeConfigSources(defaultConfig, envConfig) as RigoriumRawConfig;
  ...
  const memory = parseMemoryConfig(rawConfig.memory, diagnostics, getRigoriumMemoryRootDir(rigoriumHome), model);
  ...
  if (router?.scenarios?.default && agent.model.id !== router.scenarios.default.id) {
    ...
    // (see issue: customer reinstall doesn't help because rigorium.yaml
...
function validateTopLevel(rawConfig: RigoriumRawConfig, diagnostics: RigoriumConfigDiagnostic[]): void {
  ...
  if ("rigorium" in rawConfig) {
    diagnostics.push({
      code: "CONFIG_RIGORIUM_SECTION_FORBIDDEN",
      severity: "fatal",
      message: "YAML config must not contain a rigorium section. Use RIGORIUM_HOME for RigoriumHome.",
      path: "rigorium",
      recoverable: false,
    });
  }
  ...
  const allowedKeys = new Set([
    ...
    "tools",
    "proxy",
    // Reserved namespace for ui/server (Web UI Express bridge). The Rigorium
    // gateway does not parse `webui.*` itself but tolerates it so a single
    // ~/.rigorium/rigorium.yaml can carry both gateway-side and ui-side
...
function parseAgentThinking(value: unknown): RigoriumAgentConfig["thinking"] | undefined {
...
function parseAgentSubagents(
  value: unknown,
  diagnostics: RigoriumConfigDiagnostic[],
): RigoriumAgentConfig["subagents"] | undefined {
  if (value === undefined) {
    return undefined;
  }
  if (!isRecord(value)) {
    throw new RigoriumConfigError("CONFIG_AGENT_SUBAGENTS_INVALID", "agent.subagents must be an object.");
  }
...
function parseExtension(rawExtension: unknown, diagnostics: RigoriumConfigDiagnostic[]): RigoriumExtensionConfig {
  const defaults: RigoriumExtensionConfig = {
...
function readOptionalPositiveInteger(value: unknown, path: string): number | undefined {
  ...
  if (typeof value !== "number" || !Number.isFinite(value) || value <= 0) {
    throw new RigoriumConfigError("CONFIG_INVALID_VALUE", `${path} must be a positive integer.`);
  }
  return Math.floor(value);
}

function throwConfigErrorIfFatal(diagnostics: RigoriumConfigDiagnostic[]): void {
  const fatalDiagnostics = diagnostics.filter((diagnostic) => diagnostic.severity === "fatal");
  if (fatalDiagnostics.length > 0) {
    throw new RigoriumConfigError(
...
function parseTelemetryConfig(raw: unknown): RigoriumTelemetryConfig {
  return {
...
function parseProxyConfig(
  rawConfig: RigoriumRawConfig,
  diagnostics: RigoriumConfigDiagnostic[],
): RigoriumProxyConfig | undefined {
      ...
      if (typeof runtime.httpsProxy === "string" && runtime.httpsProxy) {
        ...
        diagnostics.push({
          ...
          message:
            ...
            "Please update your rigorium.yaml to use proxy.url instead.",
