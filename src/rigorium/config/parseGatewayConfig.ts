...
import type {
  RigoriumAdaptersConfig,
  RigoriumConfigDiagnostic,
  RigoriumGatewayConfig,
  RigoriumPlatformAdapterConfig,
} from "./types.js";

export function parseGatewayConfig(rawGateway: unknown, diagnostics: RigoriumConfigDiagnostic[]): RigoriumGatewayConfig | undefined {
  ...
  if (rawGateway.tokenPath !== undefined) {
    diagnostics.push({
      ...
      severity: "warning",
      message: "gateway.tokenPath is no longer configurable; the gateway token is stored under RigoriumHome.",
...
export function parseAdaptersConfig(rawAdapters: unknown, diagnostics: RigoriumConfigDiagnostic[]): RigoriumAdaptersConfig | undefined {
  ...
  const result: RigoriumAdaptersConfig = {
...
function parsePlatformAdapter(raw: unknown): RigoriumPlatformAdapterConfig | undefined {
  if (!isRecord(raw)) return undefined;
...
function parseFeishu(raw: unknown): RigoriumAdaptersConfig["feishu"] {
  if (!isRecord(raw)) {
    return undefined;
  }
...
function parseQQ(raw: unknown): RigoriumAdaptersConfig["qq"] {
  if (!isRecord(raw)) return undefined;
