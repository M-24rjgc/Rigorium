...
import type { RigoriumHookSyncOutput } from "../../extension/hooks/protocol/output.js";
...
export const GATEWAY_PERMISSION_CALLBACK_NAME = "rigorium.gateway.permission";

export type CreateGatewayPermissionHookOptions = {
  /** Rigorium session this hook owns. Used to scope bus pending entries. */
  sessionKey: string;
...
export function createGatewayPermissionHook(
  ...
  return async ({ hookInput, signal }) => {
    ...
    if (!delivered) {
      ...
      } satisfies RigoriumHookSyncOutput;
    }
