...
import type { RigoriumToolAuditRecorder } from "../audit/ToolAuditRecorder.js";
import type { RigoriumElicitationChannel } from "../elicitation/RigoriumElicitationChannel.js";
import type { RigoriumToolInputSchema, RigoriumToolValidationResult } from "./schema.js";
 ...
 */
export type RigoriumToolFileHistorySink = {
 ...
 * dependency cycle.
 */
export type RigoriumToolModelClient = {
 ...
 */
export type RigoriumSubagentForkApi = {
  depth: number;
...
export type RigoriumToolResultContent =
...
export type RigoriumReadFileStateEntry = {
  mtimeMs: number;
...
export type RigoriumReadFileStateMap = Map<string, RigoriumReadFileStateEntry>;

export type RigoriumWriteSnapshotEntry = {
  absolutePath: string;
  mtimeMs: number;
  contentHash: string;
...
export type RigoriumWriteSnapshotMap = Map<string, RigoriumWriteSnapshotEntry>;

export type RigoriumFileUpdateNotification = {
  absolutePath: string;
  relativePath: string;
  root: string;
  content: string;
...
export type RigoriumFileUpdateNotifier = {
  didChange?(update: RigoriumFileUpdateNotification): Promise<void> | void;
  didSave?(update: RigoriumFileUpdateNotification): Promise<void> | void;
};

export type RigoriumToolSupplementalMessage = {
  role: "user";
  content: RigoriumToolResultContent[];
  isMeta?: boolean;
};

export type RigoriumToolExecutionOutput<Output = unknown> = {
  content: RigoriumToolResultContent[];
...
export type RigoriumToolAvailability =
  | { ok: true }
  | { ok: false; code: "setup_required" | "unavailable" | "failed_check"; reason: string };

export type RigoriumToolAvailabilityContext = {
  cwd: string;
  env?: NodeJS.ProcessEnv;
};

/**
 * Tool progress event emitted via `RigoriumToolRuntimeContext.progress`.
 ...
 */
export type RigoriumToolProgressEvent = {
  type: "tool_progress";
  sessionId: string;
  turnId: string;
  toolCallId: string;
...
export type RigoriumToolProgressSink = (event: RigoriumToolProgressEvent) => void;

export type RigoriumTodoItem = {
  id?: string;
  content: string;
...
export type RigoriumTodoUpdate = {
  id?: string;
  content?: string;
  status?: RigoriumTodoItem["status"];
  priority?: string;
};

export type RigoriumTodoDiagnostics = {
  writeCount: number;
  todoCount: number;
  activeCount: number;
...
export type RigoriumTodoWriteHistoryEntry = {
  createdAt: string;
  mode: "markdown" | "structured";
  merge: boolean;
  reason?: string;
  markdown?: string;
  todos: RigoriumTodoItem[];
  diagnostics: RigoriumTodoDiagnostics;
};

export type RigoriumPlanTodoStateSnapshot = {
  approvedPlan?: string;
  requiresInitialization: boolean;
  toolCallsSinceLastTodoWrite: number;
  lastMarkdown?: string;
  todos: RigoriumTodoItem[];
  activeTodos: RigoriumTodoItem[];
  todoHistory: RigoriumTodoWriteHistoryEntry[];
  todoDiagnostics: RigoriumTodoDiagnostics;
};

export type RigoriumPlanTodoStateHandle = {
  getSnapshot(): RigoriumPlanTodoStateSnapshot;
  markPlanApproved(plan: string): void;
  recordTodoWrite(markdown: string, todos: RigoriumTodoItem[], options?: { reason?: string }): RigoriumTodoItem[];
  writeTodos(todos: RigoriumTodoUpdate[], options?: { markdown?: string; merge?: boolean; reason?: string }): RigoriumTodoItem[];
...
export type RigoriumToolRuntimeContext = {
  sessionId: string;
  turnId: string;
  cwd: string;
  abortSignal?: AbortSignal;
   ...
   * supplying a sink.
   */
  progress?: RigoriumToolProgressSink;
  /**
   ...
   */
  fileHistory?: RigoriumToolFileHistorySink;
  /**
   ...
   */
  subagent?: RigoriumSubagentForkApi;
  /**
   ...
   */
  planTodo?: RigoriumPlanTodoStateHandle;
  /**
   ...
   */
  executeTool?: (
    call: RigoriumToolCall,
    contextPatch?: Partial<RigoriumToolRuntimeContext>,
  ) => Promise<import("./result.js").RigoriumToolResult>;
  /**
   ...
   */
  fileUpdateNotifier?: RigoriumFileUpdateNotifier;
};

export type RigoriumToolDefinition<Input = unknown, Output = unknown> = {
  name: string;
  aliases?: string[];
  title?: string;
  description: string;
  kind: RigoriumToolKind;
  inputSchema: RigoriumToolInputSchema;
  ...
  validateInput?(input: Input, context: RigoriumToolRuntimeContext): Promise<RigoriumToolValidationResult>;
  checkAvailability?(context: RigoriumToolAvailabilityContext): RigoriumToolAvailability | Promise<RigoriumToolAvailability>;
  checkPermissions?(input: Input, context: RigoriumToolRuntimeContext): Promise<PermissionResult>;
  execute(input: Input, context: RigoriumToolRuntimeContext): Promise<RigoriumToolExecutionOutput<Output>>;
};

export type RigoriumToolCall = CanonicalToolCall;
