 ...
 * `docs/rigorium-deferred-feature-implementation-guide.md` §5.2 (B2) for
...
): RigoriumToolDefinition<WebFetchInput, WebFetchOutput> {
  return {
    name: "web_fetch",
    aliases: ["WebFetch"],
    ...
    execute: async (input, context): Promise<RigoriumToolExecutionOutput<WebFetchOutput>> => {
      const { url } = input;
