...
export type RigoriumMcpResourceAdapter = {
  listResources(serverId?: string): Promise<unknown>;
  readResource(serverId: string, uri: string): Promise<unknown>;
};

export function createListMcpResourcesTool(adapter?: RigoriumMcpResourceAdapter): RigoriumToolDefinition {
  return {
    ...
    execute: async (input) => {
      if (!adapter) {
        throw new RigoriumToolRuntimeError("unsupported_tool", "MCP resource adapter is not configured.");
      }
...
export function createReadMcpResourceTool(adapter?: RigoriumMcpResourceAdapter): RigoriumToolDefinition {
  return {
