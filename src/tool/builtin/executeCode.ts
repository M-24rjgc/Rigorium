  ...
  return {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        max_tool_calls: {
          type: "integer",
          description: "Maximum number of Rigorium tool calls the script may make. Defaults to 50; maximum 50.",
        },
      },
    },
    outputSchema: {
...
const UDS_PYTHON_TRANSPORT_HEADER = `"""Auto-generated Rigorium execute_code RPC helpers."""
import json
import os
import shlex
import socket
...
def _call(tool_name, args):
    ...
    while True:
        ...
        if not chunk:
            raise RuntimeError("Rigorium RPC server disconnected")
        chunks.append(chunk)
        if chunk.endswith(b"\\n"):
            break
...
const TCP_PYTHON_TRANSPORT_HEADER = `"""Auto-generated Rigorium execute_code RPC helpers."""
import json
import os
import shlex
import socket
