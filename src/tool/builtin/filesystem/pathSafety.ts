...
export function resolvePilotDeckWorkspacePath(
  ...
  if (!root) {
    ...
    return {
      ok: false,
      error: toolError("path_not_allowed", `Path ${inputPath} is outside the Rigorium workspace.`),
    };
  }
  ...
  if (options?.mustExist) {
    ...
    if (!isPathWithinRoot(real, realRoot)) {
      return {
        ok: false,
        error: toolError("path_not_allowed", `Path ${inputPath} resolves outside the Rigorium workspace.`),
      };
    }
  }
