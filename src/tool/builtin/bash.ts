...
const BASH_TOOL_DESCRIPTION = `Run a shell command in the Rigorium workspace.
...
- Do not use shell-level backgrounding (\`nohup\`, \`disown\`, \`setsid\`, trailing \`&\`) in bash. Use task_create so Rigorium can track lifecycle and output.
