...
function normalizeTodoUpdatesForFallback(todos: RigoriumTodoUpdate[]): RigoriumTodoItem[] {
...
export function parseTodoMarkdown(markdown: string): RigoriumTodoItem[] {
  ...
  return parsed.map((item, index) => {
    let status: RigoriumTodoItem["status"];
    if (item.checked) {
      status = "completed";
...
export function createTodoWriteTool(): RigoriumToolDefinition<TodoWriteInput, TodoWriteOutput> {
  return {
    name: "todo_write",
    aliases: ["TodoWrite"],
    description:
      [
        ...
        "You may use todo_write in plan mode only to organize planning work such as exploration, analysis, writing a markdown plan under `.rigorium/plans/`, and submitting that plan with `exit_plan_mode`.",
      ].join(" "),
    kind: "session",
    inputSchema: {
      type: "object",
    ...
    execute: async (input, context): Promise<RigoriumToolExecutionOutput<TodoWriteOutput>> => {
