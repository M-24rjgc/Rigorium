...
import type { RigoriumElicitationAnswer, RigoriumElicitationRequest } from "../elicitation/RigoriumElicitationChannel.js";
...
const EXIT_PLAN_MODE_DESCRIPTION =
  ...
  "Pass the plan_file_path for the markdown plan you want to submit from `.rigorium/plans`. " +
...
function getExitPlanFeedback(answer: RigoriumElicitationAnswer): string | undefined {
...
function getExitPlanAction(answer: RigoriumElicitationAnswer): "continue_planning" | "execute_plan" | undefined {
...
export function createEnterPlanModeTool(): RigoriumToolDefinition<Record<string, never>> {
  return {
...
export function createExitPlanModeTool(): RigoriumToolDefinition<ExitPlanModeInput, ExitPlanModeOutput> {
  return {
    name: "exit_plan_mode",
    ...
    inputSchema: {
      ...
      properties: {
        plan_file_path: {
          type: "string",
          description: "Path to the markdown plan file to submit from the current project's `.rigorium/plans` directory.",
        },
      },
    },
    isReadOnly: () => true,
    ...
    execute: async (input, context) => {
      ...
      if (!resolvedPlanFilePath) {
        throw new RigoriumToolRuntimeError(
          "invalid_tool_input",
          "plan_file_path must point to a markdown file under the current project's .rigorium/plans directory.",
        );
      }
      let plan: string;
      try {
      ...
      const request: RigoriumElicitationRequest = {
