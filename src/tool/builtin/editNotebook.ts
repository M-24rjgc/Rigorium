...
export function createEditNotebookTool(): RigoriumToolDefinition<EditNotebookInput, EditNotebookOutput> {
  return {
    name: "edit_notebook",
    ...
    validateInput: async (input, context) => {
      const resolved = resolveRigoriumWorkspacePath(input.notebook_path, context, { mustExist: true, forWrite: true });
      if (!resolved.ok) {
        return {
          ok: false,
          issues: [{
      ...
      try {
        ...
        const normalized = error instanceof RigoriumToolRuntimeError ? error.message : String(error);
        if (
