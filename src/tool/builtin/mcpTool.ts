...
import type { RigoriumToolInputSchema } from "../protocol/schema.js";

export type RigoriumMcpToolAdapter = {
...
export type CreateMcpToolOptions = {
  ...
  adapter?: RigoriumMcpToolAdapter;
};

export function createMcpTool(options: CreateMcpToolOptions): RigoriumToolDefinition {
  ...
  return {
    ...
    execute: async (input): Promise<RigoriumToolExecutionOutput> => {
      if (!options.adapter) {
        throw new RigoriumToolRuntimeError("unsupported_tool", "MCP adapter is not configured.");
      }
...
function normalizeMcpName(value: string): string {
  ...
  if (!normalized) {
    throw new RigoriumToolRuntimeError("invalid_tool_input", "MCP server and tool names must normalize to a name.");
  }
  return normalized;
}
