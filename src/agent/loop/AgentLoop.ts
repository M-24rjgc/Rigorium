...
export class AgentLoop {
   ...
   * `dispatchLifecycleSafely` — Stop/StopFailure/SubagentStart/SubagentStop/
   * PreModelRequest/InstructionsLoaded/PreCompact/PostCompact — and a thrown
   * hook error degrades to the empty result (hook absent), so a broken hook
   * cannot kill a turn that would otherwise succeed.
   */
  private async dispatchLifecycle(
    ...
    return dispatchLifecycleSafely(this.dependencies.lifecycle, {
      event,
      baseInput: {
        sessionId: input.sessionId,
        transcriptPath: "",
