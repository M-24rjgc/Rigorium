...
type SessionPlanTodoState = {
  ...
  lastWrite?: RigoriumTodoDiagnostics["lastWrite"];
};

export type PlanTodoStateManager = {
  forSession(sessionId: string): RigoriumPlanTodoStateHandle;
};

const TODO_WRITE_TOOL_NAME = "todo_write";
const VALID_TODO_STATUSES = new Set<RigoriumTodoItem["status"]>([
  "pending",
  "in_progress",
  "completed",
  "cancelled",
...
function normalizeTodoItem(item: RigoriumTodoUpdate, index: number): RigoriumTodoItem {
...
function dedupeById<T extends RigoriumTodoUpdate>(todos: T[]): T[] {
...
function replaceTodos(todos: RigoriumTodoUpdate[]): RigoriumTodoItem[] {
  return dedupeById(todos).map((todo, index) => normalizeTodoItem(todo, index));
}

function activeTodos(todos: RigoriumTodoItem[]): RigoriumTodoItem[] {
  return todos.filter((todo) => todo.status === "pending" || todo.status === "in_progress");
}

function cloneTodos(todos: RigoriumTodoItem[]): RigoriumTodoItem[] {
  return todos.map((todo) => ({ ...todo }));
}

function mergeTodos(existingTodos: RigoriumTodoItem[], updates: RigoriumTodoUpdate[]): RigoriumTodoItem[] {
  const existingById = new Map<string, RigoriumTodoItem>();
  ...
  const append: RigoriumTodoUpdate[] = [];
...
function buildTodoDiagnostics(state: SessionPlanTodoState): RigoriumTodoDiagnostics {
...
export function createPlanTodoStateManager(): PlanTodoStateManager {
  ...
  function snapshot(state: SessionPlanTodoState): RigoriumPlanTodoStateSnapshot {
    return {
  ...
  return {
    forSession(sessionId: string): RigoriumPlanTodoStateHandle {
      ...
      return {
        ...
        recordTodoWrite(markdown: string, todos: RigoriumTodoItem[], options?: { reason?: string }) {
          return recordWrite(state, replaceTodos(todos), { mode: "markdown", markdown, reason: options?.reason });
        },
        writeTodos(todos: RigoriumTodoUpdate[], options?: { markdown?: string; merge?: boolean; reason?: string }) {
