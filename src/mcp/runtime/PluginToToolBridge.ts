 ...
 * by an `McpRuntime`) into Rigorium `ToolDefinition`s suitable for
 ...
 *          reflected onto the Rigorium tool flags so the permission
 *          engine can decide whether to ask.
 *
 * Result transformation (M14): MCP ContentBlock types `text` and `image`
 * are mapped to their Rigorium equivalents so that images (e.g. Playwright
...
): Promise<RigoriumToolDefinition[]> {
  const tools = await runtime.listAllTools();
  return tools.map((spec) => buildToolDefinition(spec, runtime, options));
}

function buildToolDefinition(
  spec: RigoriumMcpToolSpec,
  runtime: McpRuntime,
  options: CreateToolDefinitionsOptions,
): RigoriumToolDefinition {
  const annotations: RigoriumMcpToolAnnotations = spec.annotations ?? {};
  ...
  return {
    ...
    execute: async (input, context): Promise<RigoriumToolExecutionOutput> => {
      ...
      try {
        ...
        if (err instanceof RigoriumToolRuntimeError) throw err;
...
/**
 * Map MCP `ContentBlock[]` → `RigoriumToolResultContent[]`.
 *
 ...
 */
function marshalMcpContent(raw: unknown, cwd?: string): RigoriumToolResultContent[] {
  if (!Array.isArray(raw)) return [{ type: "json", value: raw }];

  const result: RigoriumToolResultContent[] = [];
 ...
 */
function extractFileImages(text: string, cwd: string): RigoriumToolResultContent[] {
  const results: RigoriumToolResultContent[] = [];
...
function normalizeSchema(raw: unknown): RigoriumToolInputSchema {
  if (raw && typeof raw === "object") {
    const obj = raw as RigoriumToolInputSchema;
